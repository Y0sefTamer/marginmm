...
import {
  assessRefreshNeed,
  RiskAgentWorkflowError,
  runRiskAgent,
  validateCalibrationApiResponse,
  validateHederaSettlement,
  type PolicyStatus,
  type RiskAgentConfig,
} from "../src/agent.js";
import {
...
function policyStatus(state: PolicyStatus["state"] = "valid"): PolicyStatus {
  return {
    state,
    chainTimestamp: NOW,
...
function agentConfig(): RiskAgentConfig {
  return {
...
test("full calibration response validation binds signature, request and canonical evidence metadata", async () => {
  ...
  const response: CalibrationApiResponse = {
