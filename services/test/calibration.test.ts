...
import {
  ...
  validateSignedCalibration,
  validateSignedCalibrationEvidence,
} from "../src/policy-artifact.js";
...
function stressedPrices(): string[] {
  const prices = Array.from({ length: 30 }, () => "3000");
  prices[15] = "2628";
  return prices;
}

async function evidence(prices?: string[]) {
...
test("Graph errors and incomplete evidence fail closed", async () => {
  ...
  const incompleteFetch: typeof fetch = async (input) => {
