...
import {
  CalibrationIdempotencyStore,
  calibrationRequestHash,
  createCalibrationApp,
  loadCalibrationServiceConfig,
...
test("idempotency store rejects concurrent and conflicting reuse without losing a completed response", () => {
  ...
  const response = { schema: "marginmm-calibration-response-v1" } as CalibrationApiResponse;
