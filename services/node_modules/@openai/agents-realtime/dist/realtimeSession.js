...
const realtimeAgent_1 = require("./realtimeAgent.js");
...
class RealtimeSession extends _shims_1.RuntimeEventEmitter {
    ...
    async #prepareAgent(agent) {
        const [handoffs, agentTools] = await Promise.all([
            agent.getEnabledHandoffs(this.#context),
            agent.getAllTools(this.#context),
        ]);
        const handoffTools = handoffs.map((handoff) => handoff.getHandoffAsFunctionTool());
        const realtimeTools = agentTools.filter(tool_1.isValidRealtimeTool);
        const functionTools = realtimeTools.filter((tool) => tool.type === 'function');
        (0, tool_1.validateRealtimeToolNames)(functionTools, handoffs);
        const hasToolsDefined = typeof agent.tools !== 'undefined' ||
            typeof agent.mcpServers !== 'undefined';
        const hasHandoffsDefined = handoffs.length > 0;
        return {
            agent,
            toolDefinitions: hasToolsDefined || hasHandoffsDefined
                ? [...realtimeTools.map(tool_1.toRealtimeToolDefinition), ...handoffTools]
                : undefined,
            dispatchSnapshot: {
                agent,
                functionTools,
    ...
    async #getSessionConfig(additionalConfig = {}, preparedAgent, connectionGeneration) {
        ...
        const fullConfig = {
            ...
            prompt: typeof configAgent.prompt === 'function'
    ...
    #isCurrentRealtimeInvocation(invocation) {
        return (invocation.connectionGeneration === this.#connectionGeneration &&
    ...
    async #runRealtimeToolInvocation(agent, invocation, toolCall, execute) {
        ...
        if (completed) {
            if (completed.fingerprint !== invocation.fingerprint) {
                throw new agents_core_1.ModelBehaviorError(`Tool call ID ${invocation.callId} was reused for a different realtime invocation after completion.`);
            }
        ...
        if (active) {
            if (active.fingerprint !== invocation.fingerprint) {
                throw new agents_core_1.ModelBehaviorError(`Tool call ID ${invocation.callId} was reused for a different realtime invocation while execution was active.`);
            }
            const activeOutcome = await active.completion;
            if (activeOutcome &&
                invocation.connectionGeneration === this.#connectionGeneration) {
                this.#sendCommittedFunctionCallOutput(agent, invocation, toolCall, activeOutcome.output, activeOutcome.startResponse);
            }
            return;
        }
    ...
    #transferActiveToolCallToPendingApproval(agent, invocation) {
        ...
        if (active.fingerprint !== invocation.fingerprint) {
            throw new agents_core_1.ModelBehaviorError(`Tool call ID ${invocation.callId} changed while transferring realtime approval ownership.`);
        }
    ...
    async #resolveApprovalRejectionMessage(tool, toolCall, agent, toolType = 'function') {
        ...
        const perCallMessage = (0, internal_1.getToolInvocationRejectionMessage)(this.#context, agent, tool, toolCall);
        ...
        try {
            ...
            const errorDetails = logger_1.default.dontLogToolData
                ? (0, internal_1.getSafeErrorType)(error)
    ...
    async #handleFunctionToolCall(incomingToolCall, tool, agent, dispatchSnapshot) {
        ...
        const invocation = {
            ...(0, internal_1.validateToolInvocationApproval)(this.#context, agent, tool, toolCall),
            connectionGeneration: this.#connectionGeneration,
        };
        ...
        if (pending) {
            if (pending.fingerprint !== invocation.fingerprint) {
                throw new agents_core_1.ModelBehaviorError(`Tool call ID ${invocation.callId} was reused for a different realtime invocation while approval was pending.`);
            }
            return;
        }
    ...
    async #executeFunctionToolCall(toolCall, tool, agent, dispatchSnapshot, invocation) {
        ...
        const dynamicApprovalPolicy = (0, internal_1.hasDynamicFunctionToolApprovalPolicy)(tool);
        ...
        const forceApproval = dynamicApprovalPolicy &&
            (argumentParseError !== undefined ||
                !(0, internal_1.hasInspectableFunctionToolArguments)(parsedArgs));
        const existingApproval = dynamicApprovalPolicy
            ? (0, internal_1.getToolInvocationApproval)(this.context, agent, tool, toolCall)
            : undefined;
        ...
        if (needsApproval) {
            ...
            if (approval === false) {
                ...
                const trustedApprovalItem = new agents_core_1.RunToolApprovalItem(trustedToolCall, agent);
                const approvalItem = new agents_core_1.RunToolApprovalItem({ ...trustedToolCall }, agent);
    ...
    async #handleFunctionCall(incomingToolCall, dispatchSnapshot) {
        ...
        const validatedInvocation = candidateFunctionTool
            ? (0, internal_1.validateToolInvocationApproval)(this.#context, dispatchSnapshot.agent, candidateFunctionTool, toolCall)
            : candidateHandoff
                ? (0, internal_1.validateHandoffToolInvocation)(this.#context, dispatchSnapshot.agent, candidateHandoff.toolName, toolCall)
                : (0, internal_1.validateToolInvocationName)(this.#context, dispatchSnapshot.agent, toolCall.name, toolCall);
        const invocation = {
            ...validatedInvocation,
            connectionGeneration: this.#connectionGeneration,
        };
        ...
        await this.#runRealtimeToolInvocation(dispatchSnapshot.agent, invocation, toolCall, async () => {
            const [toolEnabled, handoffEnabled] = await Promise.all([
                Promise.all(dispatchSnapshot.functionTools.map((tool) => tool.isEnabled(this.#context, dispatchSnapshot.agent))),
                Promise.all(dispatchSnapshot.handoffs.map((handoff) => handoff.isEnabled({
                    runContext: this.#context,
            ...
            (0, tool_1.validateRealtimeToolNames)(filteredSnapshot.functionTools, filteredSnapshot.handoffs);
            const functionToolMap = new Map(filteredSnapshot.functionTools.map((tool) => [tool.name, tool]));
            const handoffMap = new Map(filteredSnapshot.handoffs.map((handoff) => [
                handoff.toolName,
                handoff,
            ]));
    ...
    async #runOutputGuardrails(output, responseId, itemId, sourceAgent, source, responseGeneration, connectionGeneration) {
        ...
        if (firstTripwireTriggered) {
            ...
            const error = new agents_core_1.OutputGuardrailTripwireTriggered(`Output guardrail triggered: ${(0, guardrail_1.formatRealtimeGuardrailOutputInfo)(firstTripwireTriggered.output.outputInfo)}`, firstTripwireTriggered);
            this.emit('guardrail_tripped', this.#context, sourceAgent, error, {
                itemId,
            });
            if (responseGeneration !== this.#responseGeneration ||
    ...
    #scheduleOutputGuardrails(output, responseId, itemId, source, sourceAgent, responseGeneration = this.#responseGeneration, connectionGeneration = this.#connectionGeneration) {
        void this.#runOutputGuardrails(output, responseId, itemId, sourceAgent, source, responseGeneration, connectionGeneration).catch((error) => {
    ...
    #handleOutputGuardrailDelta(event, source) {
        ...
        const sourceAgent = this.#responseDispatchSnapshots.get(responseId)?.agent ??
            this.#currentAgent;
    ...
    #setEventListeners() {
        ...
        this.#transport.on('turn_done', (event) => {
            ...
            const sourceAgent = this.#captureResponseDispatchSnapshot(responseId)?.agent ??
                this.#currentAgent;
        ...
        this.#transport.on('function_call', async (event) => {
            try {
                ...
                if (!event.responseId) {
                    throw new agents_core_1.ModelBehaviorError('Realtime function call is missing a responseId and cannot be dispatched safely.');
                }
                const dispatchSnapshot = this.#captureResponseDispatchSnapshot(event.responseId);
                if (!dispatchSnapshot) {
                    throw new agents_core_1.ModelBehaviorError('Realtime tool dispatch is unavailable before the session tool configuration is resolved.');
                }
                await this.#handleFunctionCall(event, dispatchSnapshot);
            }
            catch (error) {
                (0, internal_1.logToolActionError)(logger_1.default, 'Error handling function call', error);
                this.emit('error', {
                    type: 'error',
                    error,
                });
        ...
        this.#transport.on('mcp_approval_request', (approvalRequest) => {
            try {
                ...
                const trustedApprovalItem = (0, utils_2.realtimeApprovalItemToApprovalItem)(agent, approvalRequest);
                if (trustedApprovalItem.rawItem.type !== 'hosted_tool_call') {
                    throw new agents_core_1.ModelBehaviorError('Realtime MCP approval could not be normalized safely.');
                }
                const toolName = trustedApprovalItem.rawItem.name;
                const invocation = (0, internal_1.validateToolInvocationName)(this.#context, agent, toolName, trustedApprovalItem.rawItem);
                ...
                if (existing) {
                    if (existing.fingerprint !== invocation.fingerprint) {
                        throw new agents_core_1.ModelBehaviorError(`Tool call ID ${invocation.callId} was reused for a different realtime MCP approval invocation.`);
                    }
                    return;
                }
                const approvalItem = (0, utils_2.realtimeApprovalItemToApprovalItem)(agent, approvalRequest);
                const issued = {
                    kind: 'mcp',
                    connectionGeneration: this.#connectionGeneration,
                    agent,
                ...
                (0, internal_1.logToolActionError)(logger_1.default, 'Error handling MCP approval request', error);
                this.emit('error', {
                    type: 'error',
                    error,
                });
    ...
    #validateIssuedApproval(approvalItem) {
        ...
        if (!issued &&
            ...
            approvalItem.agent instanceof realtimeAgent_1.RealtimeAgent) {
            const agent = approvalItem.agent;
            const { callId } = (0, internal_1.validateToolInvocationName)(this.#context, agent, approvalItem.rawItem.name, approvalItem.rawItem);
        ...
        if (issued !== undefined &&
            ...
            throw new agents_core_1.ModelBehaviorError('Tool approval belongs to a closed realtime connection.');
        }
        if (!issued) {
            return undefined;
        }
        if (approvalItem.agent !== issued.agent) {
            throw new agents_core_1.ModelBehaviorError(`Tool call ID ${issued.callId} approval does not belong to its issuing realtime agent.`);
        }
        if (!('name' in approvalItem.rawItem) ||
            approvalItem.rawItem.name !== issued.rawName) {
            throw new agents_core_1.ModelBehaviorError(`Tool call ID ${issued.callId} approval changed its issued realtime tool name.`);
        }
        const supplied = issued.tool
            ? (0, internal_1.validateToolInvocationApproval)(this.#context, issued.agent, issued.tool, approvalItem.rawItem)
            : (0, internal_1.validateToolInvocationName)(this.#context, issued.agent, issued.toolName, approvalItem.rawItem);
        if (supplied.callId !== issued.callId ||
            supplied.fingerprint !== issued.fingerprint) {
            throw new agents_core_1.ModelBehaviorError(`Tool call ID ${issued.callId} approval does not match its issued realtime invocation.`);
        }
        return issued;
    }
    #resolvePendingFunctionCall(approvalItem) {
        ...
        const agent = approvalItem.agent instanceof realtimeAgent_1.RealtimeAgent
            ? approvalItem.agent
            : this.#currentAgent;
        ...
        if (pending) {
            if (pending.connectionGeneration !== this.#connectionGeneration) {
                throw new agents_core_1.ModelBehaviorError(`Tool call ID ${toolCall.callId} approval belongs to a closed realtime connection.`);
            }
            const suppliedInvocation = (0, internal_1.validateToolInvocationApproval)(this.#context, agent, pending.tool, toolCall);
            if (toolCall.name !== pending.toolCall.name ||
                suppliedInvocation.fingerprint !== pending.fingerprint) {
                throw new agents_core_1.ModelBehaviorError(`Tool call ID ${toolCall.callId} approval does not match the pending realtime invocation.`);
            }
            return pending;
        }
        ...
        const dispatchSnapshot = this.#currentDispatchSnapshot?.agent === agent
        ...
        return {
            ...
            fingerprint: invocation.fingerprint,
            connectionGeneration: invocation.connectionGeneration,
            approvalItem: toolCall === approvalItem.rawItem
                ? approvalItem
                : new agents_core_1.RunToolApprovalItem(toolCall, approvalItem.agent, approvalItem.toolName),
        };
    }
    /**
    ...
    async approve(approvalItem, options = { alwaysApprove: false }) {
        ...
        if (issued?.kind === 'function' && !pending) {
            throw new agents_core_1.ModelBehaviorError(`Tool call ID ${issued.callId} no longer has a pending realtime invocation.`);
        }
        ...
        const toolName = effectiveApprovalItem.toolName ??
        ...
        if (pending) {
            ...
            const mcpApprovalRequest = (0, utils_2.approvalItemToRealtimeApprovalItem)(effectiveApprovalItem);
            if (issued) {
    ...
    async reject(approvalItem, options = {
        ...
        if (pending) {
            ...
            const mcpApprovalRequest = (0, utils_2.approvalItemToRealtimeApprovalItem)(effectiveApprovalItem);
            const rejectionReason = (0, internal_1.getBoundToolInvocationRejectionMessage)(this.#context, effectiveApprovalItem.agent, (0, internal_1.getHostedMcpApprovalToolName)(effectiveApprovalItem.toolName ?? effectiveApprovalItem.rawItem.name, effectiveApprovalItem.rawItem), effectiveApprovalItem.rawItem);
            if (issued) {
                issued.decisionState = 'sending';
            }
            try {
