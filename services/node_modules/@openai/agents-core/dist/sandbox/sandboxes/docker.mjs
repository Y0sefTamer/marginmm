...
import { SandboxConfigurationError, SandboxLifecycleError, SandboxMountError, SandboxUnsupportedFeatureError, SandboxWorkspaceReadNotFoundError, } from "../errors.mjs";
...
import { assertLocalWorkspaceManifestMetadataSupported, joinSandboxLogicalPath, materializeLocalWorkspaceManifest, materializeLocalWorkspaceManifestMounts, pathExists, } from "./shared/localWorkspace.mjs";
import { assertHostPathGrantsRebound, assertMountCredentialsRebound, mergeManifestEntryDelta, mergeManifestDelta, sanitizeEnvironmentForPersistence, serializeHostPathGrantRedactionMetadata, serializeMountCredentialRedactionMetadata, serializeManifest, } from "./shared/manifestPersistence.mjs";
import { assertExistingMountTopologyPreserved, assertSandboxSessionStateUsable, assertSandboxStateGenerationUnchanged, captureLiveMountCredentialAuthority, captureSandboxStateGeneration, copyValidatedMountEffectivePaths, isSandboxSessionStateUnsafe, liveMountEnvironmentAuthorityMatches, manifestHasInContainerMounts, mountCredentialFileReferences, markSandboxSessionStateUnsafe, recordLiveMountCredentialAuthority, sanitizeMountCredentialEnvironmentForPersistence, validateMountCredentialBoundariesAtEffectivePath, validateMountCredentialBoundaries, validateMountCredentialFileEffectivePaths, validateMountEnvironmentCredentialBoundaries, withExclusiveSandboxManifestMutation, } from "../mountSecurity.mjs";
...
import { normalizePosixPath } from "../shared/posixPath.mjs";
import { probeSandboxDirectoryExists, probeSandboxPathExists, } from "../shared/pathProbe.mjs";
import { deserializeLocalSandboxSessionStateValues, normalizeExposedPorts, } from "./shared/sessionStateValues.mjs";
import { isStringRecord, readOptionalNumberArray, readOptionalString, readString, readStringArray, } from "../shared/typeGuards.mjs";
import { validateCredentialPair as validateSandboxCredentialPair } from "../shared/credentials.mjs";
import { sandboxPathGrantHostPath } from "../shared/hostPath.mjs";
import { getRunStateSessionTrustedConfig, isRunStateSessionState, } from "../internal/sessionStateTrust.mjs";
...
export class DockerSandboxSession extends UnixLocalSandboxSession {
    ...
    async materializeDockerInContainerEntry(args) {
        const nextManifest = mergeManifestEntryDelta(this.state.manifest, this.resolveLogicalPath(args.path), args.entry);
        assertExistingMountTopologyPreserved(this.state.manifest, nextManifest);
        validateMountCredentialBoundaries(nextManifest);
        validateMountEnvironmentCredentialBoundaries(nextManifest, this.state.environment);
        ...
        try {
            ...
            copyValidatedMountEffectivePaths(nextManifest, this.state.manifest, nextManifest);
            this.state.manifest = nextManifest;
            captureLiveMountCredentialAuthority(this.state.manifest, this.state.environment);
        }
        catch (error) {
    ...
    async applyDockerManifestExclusive(manifest, runAs) {
        ...
        try {
            ...
            const committedManifest = mergeDockerIdentityMetadata(mergeManifestDelta(this.state.manifest, materializedManifest), manifest);
            copyValidatedMountEffectivePaths(committedManifest, this.state.manifest, manifest);
            ...
            if (manifestHasInContainerMounts(manifest)) {
                ...
                recordLiveMountCredentialAuthority(this.state.manifest, previousManifest);
            }
        }
        catch (error) {
            if (manifestHasInContainerMounts(manifest) &&
    ...
    async readDockerFileAs(path, runAs) {
        const output = await this.runReadableDockerFilesystemCommand(`base64 -- ${shellQuote(path)}`, { runAs }, `read file ${path}`, path);
    ...
    async runReadableDockerFilesystemCommand(command, options, action, path) {
        ...
        if (result.status !== 0) {
            if (result.status === 1 &&
                ...
                if (!exists) {
                    throw new SandboxWorkspaceReadNotFoundError(`DockerSandboxClient path does not exist: ${path}`, { path });
                }
            }
    ...
    async runDockerMountCommand(command, action, options = {}) {
        ...
        if (result.status !== 0) {
            throw new UserError(`DockerSandboxClient failed to ${action} with exit status ${result.status}.`);
        }
        return result.stdout;
    }
...
export class DockerSandboxClient {
    ...
    async create(args, manifestOptions) {
        ...
        try {
            ...
            try {
                ...
                throw new SandboxLifecycleError('Docker sandbox creation failed and cleanup could not complete.', { errors: [error, cleanupError] });
            }
            throw error;
        }
        return session;
    ...
    async resume(state, options = {}) {
        ...
        validateMountEnvironmentCredentialBoundaries(resumeState.manifest, resumeState.environment);
        await ensureDockerAvailable();
        await this.retryFailedCreateCleanups();
    ...
    validateSessionStateForResume(input, options = {}) {
        const configuredExposedPorts = input.source === 'explicit'
            ...
            : normalizeExposedPorts(readOptionalNumberArray(input.state.configuredExposedPorts));
    ...
    async canReusePreservedOwnedSession(state, options = {}) {
        ...
        if (!liveMountEnvironmentAuthorityMatches(state.manifest, state.manifest, state.environment)) {
            return false;
        }
        const resolvedOptions = {
            ...this.options,
        ...
        if (options.revalidateManifestEntries === true) {
            ...
            if (state.materializedEntriesFingerprint !==
                ...
                throw new UserError('Docker sandbox resume cannot apply changes to non-mount manifest entries. Start a fresh sandbox session instead.');
            }
        }
        let inspection;
        try {
    ...
    async serializeSessionState(state, options = {}) {
        ...
        validateMountEnvironmentCredentialBoundaries(state.manifest, state.environment);
        ...
        if (options.preserveOwnedSession === true &&
            ...
            snapshot === null) {
            throw new UserError('Docker sandbox session cannot be preserved after live reuse was rejected because no restorable snapshot is configured.');
        }
        const sanitizedMountEnvironment = sanitizeMountCredentialEnvironmentForPersistence(state);
        const serialized = {
    ...
    async deserializeSessionState(state) {
        const configuredExposedPorts = normalizeExposedPorts(readOptionalNumberArray(state.configuredExposedPorts));
        const networkMode = validateDockerNetworkConfiguration(state.networkMode, configuredExposedPorts);
        const baseState = await deserializeLocalSandboxSessionStateValues(state, this.options.snapshot);
        return {
            ...baseState,
        ...
        if (workspaceExists) {
            if (containerRunning) {
                ...
                if (inspection.ownershipVerified &&
                    ...
                    throw new UserError('Existing Docker sandbox labels do not match required labels. Start a fresh sandbox session instead.');
                }
                if (!inspection.ownershipVerified) {
                    throw new UserError('Docker sandbox container identity could not be verified. Refusing to resume or replace the running container.');
                }
            ...
            if (await localSnapshotIsRestorable(state)) {
                const restoredState = await restoreLocalSnapshotToWorkspace(state, state.workspaceRootPath, { archiveLimits });
                await this.cleanupDockerResources(state);
    ...
    async restoreSnapshotIntoNewWorkspace(state, archiveLimits, workspaceBaseDir = this.options.workspaceBaseDir) {
        const workspaceRootPath = await mkdtemp(join(workspaceBaseDir ?? tmpdir(), 'openai-agents-docker-sandbox-'));
        try {
    ...
    async restartContainer(state, workspaceRootPath, archiveLimits) {
        ...
        try {
            ...
            try {
                ...
                throw new SandboxLifecycleError('Docker sandbox restart failed and cleanup could not complete.', { errors: [error, cleanupError] });
            }
            throw error;
        }
        this.failedCreateCleanupTokens.delete(container.containerId);
...
async function dockerNetworkConfigurationMatches(state) {
    ...
    const inspectionResult = await runSandboxProcess('docker', [
        'inspect',
        '--type',
        'container',
        '--format',
...
async function resolveDockerPathGrantMounts(manifest) {
    return await Promise.all(manifest.extraPathGrants.map(async (grant) => await resolveDockerMountAuthority(sandboxPathGrantHostPath(grant), grant.path, grant.readOnly)));
}
...
function normalizeDockerLabels(value, source) {
    ...
    for (const key of Object.keys(labels)) {
        if (RESERVED_DOCKER_LABELS.has(key)) {
            throw new UserError(`${source} cannot override reserved SDK label "${key}".`);
        }
    }
    return labels;
}
...
function resolveDockerLabels(clientOptions, perRunOptions, explicitStateFallback) {
    ...
    if (trustedLabels !== undefined &&
        ...
        throw new UserError('DockerSandboxClient labels cannot be changed when resuming explicit session state. Start a fresh sandbox session instead.');
    }
...
function validateDockerNetworkConfiguration(networkMode, exposedPorts) {
    if (networkMode !== undefined && networkMode !== 'none') {
        throw new UserError('DockerSandboxClient networkMode must be "none" when provided.');
    }
    if (networkMode === 'none' && (exposedPorts?.length ?? 0) > 0) {
        throw new UserError('DockerSandboxClient exposedPorts cannot be used when networkMode is "none".');
    }
    return networkMode;
}
function resolveDockerNetworkConfiguration(clientOptions, perRunOptions, explicitStateFallback) {
    ...
    if (explicitStateFallback &&
        ...
        throw new UserError('DockerSandboxClient exposedPorts cannot be changed when resuming explicit session state. Start a fresh sandbox session instead.');
    }
...
function assertDockerManifestDeltaSupported(currentManifest, deltaManifest) {
    ...
    if (splitGrant) {
        throw new SandboxUnsupportedFeatureError(`DockerSandboxClient cannot add or change path grant hostPath for "${splitGrant.path}" on an already-running container. Configure it in the manifest passed to create().`, {
            provider: 'DockerSandboxClient',
            feature: 'manifest.extraPathGrants.hostPath',
            path: splitGrant.path,
        });
    }
    assertLocalWorkspaceManifestMetadataSupported('DockerSandboxClient', deltaManifest, {
        allowLocalBindMounts: false,
        allowIdentityMetadata: true,
        supportsMount: isSupportedDockerApplyMount,
    });
...
async function inspectContainerRunning(containerId) {
    ...
    if (result.status !== 0) {
        ...
        throw new UserError(`Failed to inspect Docker sandbox container: ${formatSandboxProcessError(result)}`);
    }
    ...
    throw new UserError(`Failed to inspect Docker sandbox container: unexpected running state "${running}".`);
}
...
async function prepareDockerInContainerMount(session, logicalPath, entry, manifest, credentialBoundaryManifest = manifest, environment = session.state.environment) {
    ...
    const prepareCandidate = async () => {
        ...
        const credentialExposure = validateMountCredentialBoundariesAtEffectivePath(manifest, logicalPath, entry, effectiveMountPath);
    ...
    return {
        ...
        revalidateMountAuthority: async () => {
            ...
            if (current.effectiveMountPath !== candidate.effectiveMountPath ||
                ...
                throw new SandboxMountError('Docker sandbox mount authority changed after validation. Retry from current trusted configuration.', { mountType: entry.type }, 'mount_config_invalid');
            }
        },
    };
}
async function prepareDockerMountCredentialFiles(session, manifest, entry, mountPath, environment) {
    const references = mountCredentialFileReferences(manifest, entry, environment);
    ...
    const resolveEffectivePath = async (path) => {
        ...
        if (!resolvedPath.startsWith('/') || resolvedPath.includes('\n')) {
            throw new SandboxMountError('DockerSandboxClient failed to resolve an absolute mount credential path.', { mountType: entry.type }, 'mount_config_invalid');
        }
    ...
    const resolvedReferences = await Promise.all(references.map(async ({ field, path }) => {
        if (field === 'mountStrategy.pattern.configFilePath') {
            try {
                ...
                throw new SandboxMountError('DockerSandboxClient failed to resolve the rclone config file.', { mountType: entry.type }, 'mount_config_invalid');
            }
        }
...
async function resolveDockerEffectiveMountPath(session, logicalPath, entry, declaredMountPath) {
    ...
    if (workspaceRelativePath === null) {
        ...
        if (!resolvedPath.startsWith('/') || resolvedPath.includes('\n')) {
            throw new SandboxMountError('DockerSandboxClient failed to resolve an absolute mount path.', { mountType: entry.type }, 'mount_config_invalid');
        }
        return normalizePosixPath(resolvedPath);
    }
    const workspaceRoot = await realpath(session.state.workspaceRootPath);
    const effectiveHostPath = await realpathWithMissingTail(resolve(session.state.workspaceRootPath, workspaceRelativePath));
    ...
    if (effectiveWorkspacePath === '..' ||
        ...
        throw new SandboxMountError('DockerSandboxClient mount path resolves outside the sandbox workspace.', { mountType: entry.type }, 'mount_config_invalid');
    }
    return joinSandboxLogicalPath(session.state.manifest.root, effectiveWorkspacePath.split(sep).join('/'));
}
...
async function cleanupDockerAppliedMounts(session, targets) {
    ...
    if (cleanupErrors.length > 0) {
        throw new SandboxMountError('DockerSandboxClient credential-bearing mount cleanup failed.', {
            provider: 'docker',
...
async function throwAfterDockerAppliedMountCleanup(session, targets, mountError) {
    try {
        ...
        throw new SandboxMountError('DockerSandboxClient mount failed and credential cleanup could not complete.', {
            provider: 'docker',
...
async function applyDockerMountpointMount(session, entry, mountPath, pattern, allowAmbientCredentials, environment, revalidateMountAuthority) {
    ...
    const hasExplicitCredentials = entry.type === 's3_mount'
        ...
        : Boolean(readOptionalString(entry, 'accessId') &&
    ...
    const command = envText
        ...
        : shellCommand(mountArgs);
    await revalidateMountAuthority();
    await session.runDockerMountCommand([`mkdir -p -- ${shellQuote(mountPath)}`, command].join(' && '), `mount mountpoint ${entry.type}`, envText
        ? {
            input: envText,
...
async function applyDockerS3FilesMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority) {
    if (!allowAmbientCredentials) {
        throw new SandboxMountError('DockerSandboxClient s3files mounts require explicit trusted credential exposure for their workload identity provider.', { mountPath, mountType: entry.type }, 'mount_config_invalid');
    }
    if (entry.type !== 's3_files_mount') {
...
async function applyDockerAzureBlobFuseMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority) {
    ...
    if (!entry.accountKey &&
        ...
        throw new SandboxMountError('DockerSandboxClient blobfuse mounts require explicit trusted credential exposure before using managed identity.', { mountPath, mountType: entry.type }, 'mount_config_invalid');
    }
...
function dockerMountArgsForManifest(manifest, containerName, manifestBindMounts) {
    ...
    for (const { mountPath, entry, } of manifest.mountTargetsForMaterialization()) {
        if (isDockerBindMount(entry)) {
            ...
            if (!bindMount) {
                throw new SandboxConfigurationError(`Docker bind mount authority was not resolved for "${mountPath}".`);
            }
...
async function dockerRcloneMountConfig(session, entry, pattern, mountPath, allowAmbientCredentials) {
    ...
    const withConfigText = async (config) => {
        ...
        const encodedConfig = await session.runDockerMountCommand(`base64 -- ${shellQuote(sourcePath)}`, 'read rclone config');
