...
const DOCKER_MOUNT_AUTHORITY_FINGERPRINT_LABEL = 'openai-agents-sandbox.mount-authority-fingerprint';
...
class DockerSandboxSession extends unixLocal_1.UnixLocalSandboxSession {
    ...
    stagedMountEnvironment;
    mountedPathGrants;
    constructor(args) {
        super(args);
        this.mountedPathGrants = args.state.manifest.extraPathGrants.map((grant) => ({ ...grant }));
    }
    ...
    createEditor(runAs) {
        ...
        return new DockerSandboxEditor(this, runAs, runAs ? undefined : super.createEditor());
    }
    ...
    async pathExists(path, runAs) {
        ...
        return await (0, pathProbe_1.probeSandboxPathExists)({
            path: absolutePath,
            runCommand: async (command) => await this.runDockerFilesystemCommand(command, { runAs }),
        });
    }
    async directoryExists(path, runAs) {
        const requiresContainerProbe = Boolean(dockerVolumeMountContainingPath(this.state.manifest, path) ||
            this.pathRequiresDockerFilesystem(path));
        if (!runAs && !requiresContainerProbe) {
            return await super.directoryExists(path);
        }
        const absolutePath = this.resolveContainerFilesystemPath(path);
        return await (0, pathProbe_1.probeSandboxDirectoryExists)({
            path: absolutePath,
            runCommand: async (command) => await this.runDockerFilesystemCommand(command, { runAs }),
        });
    }
    ...
    async listDir(args) {
        ...
        ].join(' && '), { runAs: args.runAs }, `list directory ${absolutePath}`, absolutePath);
        const resolvedPath = new workspacePaths_1.WorkspacePathPolicy({
    ...
    pathUsesSplitGrant(path) {
        return Boolean(dockerSplitPathGrantContainingPath(this.state.manifest, path));
    }
    async materializeEntry(args) {
        const entry = structuredClone(args.entry);
        if (!(0, entries_1.isMount)(entry) || !isDockerInContainerMount(entry)) {
            await super.materializeEntry({ ...args, entry });
            return;
        }
        await (0, mountSecurity_1.withExclusiveSandboxManifestMutation)(this.state, async () => {
            await this.materializeDockerInContainerEntry({ ...args, entry });
        });
    }
    async materializeDockerInContainerEntry(args) {
        const nextManifest = (0, manifestPersistence_1.mergeManifestEntryDelta)(this.state.manifest, this.resolveLogicalPath(args.path), args.entry);
        (0, mountSecurity_1.assertExistingMountTopologyPreserved)(this.state.manifest, nextManifest);
        (0, mountSecurity_1.validateMountCredentialBoundaries)(nextManifest);
        (0, mountSecurity_1.validateMountEnvironmentCredentialBoundaries)(nextManifest, this.state.environment);
        ...
        const preparedMount = await prepareDockerInContainerMount(this, logicalPath, args.entry, nextManifest);
        try {
            ...
            if (args.runAs) {
                ...
                await this.chownContainerPath(preparedMount.effectiveMountPath, args.runAs);
            }
            await applyPreparedDockerInContainerMount(this, preparedMount);
            (0, mountSecurity_1.copyValidatedMountEffectivePaths)(nextManifest, this.state.manifest, nextManifest);
            this.state.manifest = nextManifest;
            (0, mountSecurity_1.captureLiveMountCredentialAuthority)(this.state.manifest, this.state.environment);
        }
        catch (error) {
    ...
    async applyManifest(manifest, runAs) {
        const manifestSnapshot = (0, manifest_1.cloneManifest)(manifest);
        await (0, mountSecurity_1.withExclusiveSandboxManifestMutation)(this.state, async () => {
            await this.applyDockerManifestExclusive(manifestSnapshot, runAs);
        });
    }
    async applyDockerManifestExclusive(manifest, runAs) {
        const previousManifest = this.state.manifest;
        const nextManifest = (0, manifestPersistence_1.mergeManifestDelta)(this.state.manifest, manifest);
        (0, mountSecurity_1.assertExistingMountTopologyPreserved)(this.state.manifest, nextManifest);
        (0, mountSecurity_1.validateMountCredentialBoundaries)(nextManifest);
        ...
        (0, mountSecurity_1.validateMountEnvironmentCredentialBoundaries)(nextManifest, nextEnvironment);
        this.stagedMountEnvironment = nextEnvironment;
        let providerEffectsMayHaveStarted = false;
        try {
            const preparedMounts = await prepareDockerInContainerMounts(this, manifest, nextManifest, nextEnvironment);
            const preparedMountsByLogicalPath = new Map(preparedMounts.map((prepared) => [prepared.logicalPath, prepared]));
            ...
            if (runAs) {
                for (const [path, entry] of Object.entries(manifest.entries)) {
                    if ((0, entries_1.isMount)(entry)) {
                        const mountPath = preparedMountsByLogicalPath.get(path)?.effectiveMountPath ??
            ...
            const committedManifest = mergeDockerIdentityMetadata((0, manifestPersistence_1.mergeManifestDelta)(this.state.manifest, materializedManifest), manifest);
            (0, mountSecurity_1.copyValidatedMountEffectivePaths)(committedManifest, this.state.manifest, manifest);
            this.state.manifest = committedManifest;
            this.state.environment = nextEnvironment;
            if ((0, mountSecurity_1.manifestHasInContainerMounts)(manifest)) {
                (0, mountSecurity_1.captureLiveMountCredentialAuthority)(this.state.manifest, this.state.environment);
            }
            else {
                (0, mountSecurity_1.recordLiveMountCredentialAuthority)(this.state.manifest, previousManifest);
            }
        }
        catch (error) {
            if ((0, mountSecurity_1.manifestHasInContainerMounts)(manifest) &&
    ...
    async readDockerFileAs(path, runAs) {
        const output = await this.runReadableDockerFilesystemCommand(`base64 -- ${(0, shell_1.shellQuote)(path)}`, { runAs }, `read file ${path}`, path);
        return Buffer.from(output.replace(/\s+/gu, ''), 'base64');
    }
    async runReadableDockerFilesystemCommand(command, options, action, path) {
        ...
        if (result.status !== 0) {
            if (result.status === 1 &&
                ...
                !result.signal &&
                !result.error) {
                const exists = await (0, pathProbe_1.probeSandboxPathExists)({
                    path,
                    runCommand: async (command) => await this.runDockerFilesystemCommand(command, options),
                });
                if (!exists) {
                    throw new errors_2.SandboxWorkspaceReadNotFoundError(`DockerSandboxClient path does not exist: ${path}`, { path });
                }
            }
    ...
    async runDockerMountCommand(command, action, options = {}) {
        ...
        try {
            ...
            throw new errors_1.UserError(`DockerSandboxClient failed to ${action}.`);
        }
        if (result.status !== 0) {
            throw new errors_1.UserError(`DockerSandboxClient failed to ${action} with exit status ${result.status}.`);
        }
        return result.stdout;
    }
    ...
    async runDockerFilesystemCommand(command, options = {}) {
        ...
        for (const [key, value] of Object.entries(options.environment ?? this.state.environment)) {
            dockerArgs.push('-e', `${key}=${value}`);
        }
    ...
    async invalidateAfterFailedPrivilegedManifestTransition() {
        (0, mountSecurity_1.markSandboxSessionStateUnsafe)(this.state);
        try {
            await removeDockerContainer(this.state.containerId, {
                ignoreMissing: true,
            });
...
class DockerSandboxEditor {
    ...
    runAs;
    hostEditor;
    constructor(session, runAs, hostEditor) {
        this.session = session;
        this.runAs = runAs;
        this.hostEditor = hostEditor;
    }
    canAccessPathForEdit(path) {
        if (!this.shouldUseDockerFilesystem(path)) {
            const canAccessPathForEdit = this.hostEditor?.canAccessPathForEdit;
    ...
    async deleteFile(operation, context) {
        if (!this.shouldUseDockerFilesystem(operation.path)) {
    ...
    shouldUseDockerFilesystem(...paths) {
...
class DockerSandboxClient {
    ...
    options;
    failedCreateCleanupTokens = new Map();
    constructor(options = {}) {
        this.options = {
            ...options,
            ...(options.labels === undefined
                ? {}
                : {
                    labels: normalizeDockerLabels(options.labels, 'DockerSandboxClient labels'),
                }),
        };
    }
    async create(args, manifestOptions) {
        ...
        const { configuredExposedPorts, networkMode } = resolveDockerNetworkConfiguration(this.options, createArgs.options);
        const labels = resolveDockerLabels(this.options, createArgs.options);
        const environment = await manifest.resolveEnvironment();
        (0, mountSecurity_1.validateMountEnvironmentCredentialBoundaries)(manifest, environment);
        ...
        const sessionIdentity = (0, node_crypto_1.randomUUID)();
        ...
        const session = new DockerSandboxSession({
            state: {
                sessionIdentity,
                materializedEntriesFingerprint: dockerMaterializedEntriesFingerprint(manifest),
                manifest,
                workspaceRootPath,
        ...
        try {
            ...
            (0, mountSecurity_1.captureLiveMountCredentialAuthority)(manifest, environment);
        }
        catch (error) {
            const cleanupToken = {
                containerId: container.containerId,
            ...
            try {
                ...
                throw new errors_2.SandboxLifecycleError('Docker sandbox creation failed and cleanup could not complete.', { errors: [error, cleanupError] });
            }
            throw error;
        }
        return session;
    ...
    async resume(state, options = {}) {
        const trustedConfig = (0, sessionStateTrust_1.getRunStateSessionTrustedConfig)(state);
        ...
        if (!(0, sessionStateTrust_1.isRunStateSessionState)(state)) {
            const { configuredExposedPorts, networkMode } = resolveDockerNetworkConfiguration(this.options, options.clientOptions, state);
            if (state.networkMode !== networkMode ||
                !dockerExposedPortSetsEqual(state.configuredExposedPorts ?? [], configuredExposedPorts)) {
                resumeState = {
                    ...state,
                    configuredExposedPorts,
                    networkMode,
            ...
            const labels = resolveDockerLabels(this.options, options.clientOptions, state);
            resumeState = {
                ...resumeState,
                labels,
            };
        }
        (0, manifestPersistence_1.assertMountCredentialsRebound)(resumeState);
        (0, manifestPersistence_1.assertHostPathGrantsRebound)(resumeState);
        assertDockerManifestSupported(resumeState.manifest);
        (0, mountSecurity_1.validateMountEnvironmentCredentialBoundaries)(resumeState.manifest, resumeState.environment);
        await ensureDockerAvailable();
        await this.retryFailedCreateCleanups();
        const archiveLimits = options.archiveLimits === undefined
            ? this.options.archiveLimits
            : options.archiveLimits;
        const restoredState = await this.restoreIfNeeded(resumeState, archiveLimits);
        return new DockerSandboxSession({
            state: restoredState,
            archiveLimits,
        });
    }
    validateSessionStateForResume(input, options = {}) {
        const configuredExposedPorts = input.source === 'explicit'
            ? (0, sessionStateValues_1.normalizeExposedPorts)(input.state.configuredExposedPorts)
            : (0, sessionStateValues_1.normalizeExposedPorts)((0, typeGuards_1.readOptionalNumberArray)(input.state.configuredExposedPorts));
        const networkMode = validateDockerNetworkConfiguration(input.state.networkMode, configuredExposedPorts);
        resolveDockerNetworkConfiguration(this.options, options.clientOptions, input.source === 'explicit'
            ? { configuredExposedPorts, networkMode }
            : undefined);
        const persistedLabels = normalizeDockerLabels(input.state.labels, 'Docker sandbox session state labels');
        resolveDockerLabels(this.options, options.clientOptions, input.source === 'explicit' ? { labels: persistedLabels } : undefined);
    }
    async canReusePreservedOwnedSession(state, options = {}) {
        validateDockerNetworkConfiguration(state.networkMode, state.configuredExposedPorts);
        if ((0, mountSecurity_1.isSandboxSessionStateUnsafe)(state)) {
            return false;
        }
        if ((0, sessionStateTrust_1.isRunStateSessionState)(state)) {
            return false;
        }
        if (!(0, mountSecurity_1.liveMountEnvironmentAuthorityMatches)(state.manifest, state.manifest, state.environment)) {
            return false;
        }
        const resolvedOptions = {
            ...this.options,
        ...
        const { configuredExposedPorts, networkMode } = resolveDockerNetworkConfiguration(this.options, options.clientOptions);
        if (state.image !== (resolvedOptions.image ?? DEFAULT_DOCKER_IMAGE) ||
            !dockerExposedPortSetsEqual(state.configuredExposedPorts ?? [], configuredExposedPorts) ||
        ...
        if (options.revalidateManifestEntries === true) {
            ...
            if (state.materializedEntriesFingerprint !==
                dockerMaterializedEntriesFingerprint(state.manifest)) {
                throw new errors_1.UserError('Docker sandbox resume cannot apply changes to non-mount manifest entries. Start a fresh sandbox session instead.');
            }
        }
        let inspection;
        try {
    ...
    async serializeSessionState(state, options = {}) {
        (0, mountSecurity_1.assertSandboxSessionStateUsable)(state);
        const labels = normalizeDockerLabels(state.labels, 'Docker sandbox session state labels');
        (0, mountSecurity_1.validateMountEnvironmentCredentialBoundaries)(state.manifest, state.environment);
        const stateGeneration = (0, mountSecurity_1.captureSandboxStateGeneration)(state);
        ...
        if (options.preserveOwnedSession === true &&
            options.reuseLiveSession === false &&
            options.willCloseAfterSerialize === true &&
            snapshot === null) {
            throw new errors_1.UserError('Docker sandbox session cannot be preserved after live reuse was rejected because no restorable snapshot is configured.');
        }
        const sanitizedMountEnvironment = (0, mountSecurity_1.sanitizeMountCredentialEnvironmentForPersistence)(state);
        const serialized = {
            ...(0, manifestPersistence_1.serializeHostPathGrantRedactionMetadata)(state),
            ...(0, manifestPersistence_1.serializeMountCredentialRedactionMetadata)(state),
            manifest: (0, manifestPersistence_1.serializeManifest)(sanitizedMountEnvironment.manifest),
            ...
            environment: (0, manifestPersistence_1.sanitizeEnvironmentForPersistence)(sanitizedMountEnvironment),
        ...
        (0, mountSecurity_1.assertSandboxStateGenerationUnchanged)(state, stateGeneration);
        return serialized;
    }
    async deserializeSessionState(state) {
        const configuredExposedPorts = (0, sessionStateValues_1.normalizeExposedPorts)((0, typeGuards_1.readOptionalNumberArray)(state.configuredExposedPorts));
        const networkMode = validateDockerNetworkConfiguration(state.networkMode, configuredExposedPorts);
        const baseState = await (0, sessionStateValues_1.deserializeLocalSandboxSessionStateValues)(state, this.options.snapshot);
        return {
            ...baseState,
            sessionIdentity: (0, typeGuards_1.readOptionalString)(state, 'sessionIdentity'),
            ...
            networkMode,
            labels: normalizeDockerLabels(state.labels, 'Serialized Docker sandbox session state labels'),
            dockerVolumeNames: (0, typeGuards_1.readStringArray)(state.dockerVolumeNames),
        };
    }
    async restoreIfNeeded(state, archiveLimits) {
        attachDockerSnapshotExcludedPaths(state);
        if ((0, sessionStateTrust_1.isRunStateSessionState)(state)) {
            const trustedConfig = (0, sessionStateTrust_1.getRunStateSessionTrustedConfig)(state);
            const resolvedOptions = {
            ...
            const { configuredExposedPorts, networkMode } = resolveDockerNetworkConfiguration(this.options, trustedConfig?.clientOptions);
            const labels = resolveDockerLabels(this.options, trustedConfig?.clientOptions);
            const trustedState = {
                ...state,
                ...
                snapshotSpec: trustedConfig?.snapshot ??
            ...
            if (!(await (0, localSnapshots_1.localSnapshotIsRestorable)(trustedState))) {
                throw new errors_1.UserError('Docker sandbox resources are unavailable and no local snapshot could be restored.');
            }
            return await this.restoreSnapshotIntoNewWorkspace(trustedState, archiveLimits, resolvedOptions.workspaceBaseDir);
        }
        ...
        if (workspaceExists) {
            if (containerRunning) {
                ...
                if (inspection.ownershipVerified &&
                    ...
                    throw new errors_1.UserError('Existing Docker sandbox labels do not match required labels. Start a fresh sandbox session instead.');
                }
                if (!inspection.ownershipVerified) {
                    throw new errors_1.UserError('Docker sandbox container identity could not be verified. Refusing to resume or replace the running container.');
                }
                await this.cleanupDockerResources(state);
                return await this.restartContainer(state, state.workspaceRootPath, archiveLimits);
            }
            ...
            if (await (0, localSnapshots_1.localSnapshotIsRestorable)(state)) {
                const restoredState = await (0, localSnapshots_1.restoreLocalSnapshotToWorkspace)(state, state.workspaceRootPath, { archiveLimits });
                await this.cleanupDockerResources(state);
                return await this.restartContainer(restoredState, restoredState.workspaceRootPath, archiveLimits);
            }
        }
        if (containerRunning) {
    ...
    async restoreSnapshotIntoNewWorkspace(state, archiveLimits, workspaceBaseDir = this.options.workspaceBaseDir) {
        const workspaceRootPath = await (0, promises_1.mkdtemp)((0, node_path_1.join)(workspaceBaseDir ?? (0, node_os_1.tmpdir)(), 'openai-agents-docker-sandbox-'));
        try {
            const restoredState = await (0, localSnapshots_1.restoreLocalSnapshotToWorkspace)({
                ...state,
                workspaceRootPath,
                workspaceRootOwned: true,
            }, workspaceRootPath, { archiveLimits });
            return await this.restartContainer(restoredState, workspaceRootPath, archiveLimits);
        }
        catch (error) {
            await (0, promises_1.rm)(workspaceRootPath, { recursive: true, force: true }).catch(() => undefined);
            throw error;
        }
    }
    ...
    async restartContainer(state, workspaceRootPath, archiveLimits) {
        ...
        const sessionIdentity = state.sessionIdentity ?? (0, node_crypto_1.randomUUID)();
        const container = await startDockerContainer({
            image: state.image,
            ...
            networkMode: state.networkMode,
            labels: normalizeDockerLabels(state.labels, 'Docker sandbox session state labels'),
        });
        const nextState = {
            ...state,
            sessionIdentity,
            materializedEntriesFingerprint: dockerMaterializedEntriesFingerprint(state.manifest),
            workspaceRootPath,
            containerId: container.containerId,
            labels: normalizeDockerLabels(state.labels, 'Docker sandbox session state labels'),
            dockerVolumeNames: container.volumeNames,
        ...
        try {
            ...
            try {
                ...
                throw new errors_2.SandboxLifecycleError('Docker sandbox restart failed and cleanup could not complete.', { errors: [error, cleanupError] });
            }
            throw error;
        }
        this.failedCreateCleanupTokens.delete(container.containerId);
    ...
    async retryFailedCreateCleanups() {
...
async function resolveDockerRunStateEnvironment(state) {
    return Object.fromEntries(await Promise.all(Object.entries(state.manifest.environment).map(async ([key, value]) => [
        key,
        (0, manifest_1.isEnvValueReference)(value) && typeof state.environment[key] === 'string'
            ? state.environment[key]
            : await value.resolve(),
    ])));
}
function assertDockerManifestSupported(manifest) {
    (0, mountSecurity_1.validateMountCredentialBoundaries)(manifest);
    assertDockerManifestRootSupported(manifest);
    for (const grant of manifest.extraPathGrants) {
        (0, hostPath_1.sandboxPathGrantHostPath)(grant);
    }
...
async function inspectRunningDockerContainerForReuse(state) {
    const labels = await inspectDockerContainerLabels(state.containerId);
    ...
    const workspaceMountBeforeInspect = await resolveDockerWorkspaceMount(state.workspaceRootPath, state.manifest.root);
    const pathGrantMountsBeforeInspect = await resolveDockerPathGrantMounts(state.manifest);
    const manifestBindMountsBeforeInspect = await resolveDockerManifestBindMounts(state.manifest);
    const actualBindMounts = await inspectDockerBindMounts(state.containerId);
    const workspaceMountAfterInspect = await resolveDockerWorkspaceMount(state.workspaceRootPath, state.manifest.root);
    const pathGrantMountsAfterInspect = await resolveDockerPathGrantMounts(state.manifest);
    const manifestBindMountsAfterInspect = await resolveDockerManifestBindMounts(state.manifest);
    if (!actualBindMounts) {
        return { ownershipVerified: false, reusable: false };
    }
    ...
    const ownershipVerified = actualBindMounts.some((mount) => dockerBindMountsEqual(mount, expectedWorkspaceMount));
    ...
    const fingerprintBeforeInspect = dockerMountAuthorityFingerprint(sessionIdentity, workspaceMountBeforeInspect, pathGrantMountsBeforeInspect, manifestBindMountsBeforeInspect, state.manifest);
    const fingerprintAfterInspect = dockerMountAuthorityFingerprint(sessionIdentity, workspaceMountAfterInspect, pathGrantMountsAfterInspect, manifestBindMountsAfterInspect, state.manifest);
    return {
        ownershipVerified: true,
        reusable: fingerprintBeforeInspect === fingerprintAfterInspect &&
            recordedFingerprint === fingerprintAfterInspect &&
            dockerBindMountSetsEqual(actualBindMounts, expectedBindMounts),
    };
}
async function inspectLegacyDockerContainerForReuse(state, labels) {
    if (labels[DOCKER_OWNERSHIP_LABEL] !== 'true' ||
    ...
    const ownershipVerified = actualBindMounts.some((mount) => dockerBindMountsEqual(mount, expectedWorkspaceMount));
    const networkConfigurationMatches = ownershipVerified && (await dockerNetworkConfigurationMatches(state));
    return {
        ownershipVerified,
        reusable: networkConfigurationMatches &&
            dockerMountAuthoritiesEqual(workspaceMountBeforeInspect, workspaceMountAfterInspect) &&
            dockerMountAuthoritySetsEqual(manifestBindMountsBeforeInspect, manifestBindMountsAfterInspect) &&
...
async function dockerNetworkConfigurationMatches(state) {
    if (state.networkMode === undefined) {
        return true;
    }
    const inspectionResult = await (0, runProcess_1.runSandboxProcess)('docker', [
        'inspect',
        '--type',
        'container',
        '--format',
...
function dockerMountAuthoritySetsEqual(left, right) {
    const sortedLeft = sortDockerMountAuthorities(left);
    const sortedRight = sortDockerMountAuthorities(right);
    return (sortedLeft.length === sortedRight.length &&
        sortedLeft.every((mount, index) => dockerMountAuthoritiesEqual(mount, sortedRight[index])));
}
function sortDockerMountAuthorities(mounts) {
...
async function inspectDockerContainerLabels(containerId) {
...
async function inspectDockerBindMounts(containerId) {
    const result = await (0, runProcess_1.runSandboxProcess)('docker', [
        'inspect',
        '--type',
        'container',
    ...
    try {
        ...
        return await Promise.all(mounts
            ...
            return {
                source: await (0, promises_1.realpath)(mount.Source),
...
function dockerBindMountsEqual(left, right) {
    return (left.source === right.source &&
        left.target === right.target &&
        left.readWrite === right.readWrite);
}
function dockerBindMountsFromAuthority(mounts) {
...
function dockerBindMountSetsEqual(left, right) {
    return (JSON.stringify(sortDockerBindMounts(left)) ===
        JSON.stringify(sortDockerBindMounts(right)));
}
function sortDockerBindMounts(mounts) {
    return [...mounts].sort((left, right) => {
        const targetOrder = left.target.localeCompare(right.target);
        return targetOrder !== 0
            ? targetOrder
...
async function resolveDockerWorkspaceMount(workspaceRootPath, manifestRoot) {
    return await resolveDockerMountAuthority(workspaceRootPath, manifestRoot, false);
}
async function resolveDockerMountAuthority(sourcePath, target, readOnly) {
    const source = await (0, promises_1.realpath)(sourcePath);
    const sourceStats = await (0, promises_1.stat)(source, { bigint: true });
    return {
        source,
        sourceIdentity: {
...
async function resolveDockerPathGrantMounts(manifest) {
    return await Promise.all(manifest.extraPathGrants.map(async (grant) => await resolveDockerMountAuthority((0, hostPath_1.sandboxPathGrantHostPath)(grant), grant.path, grant.readOnly)));
}
async function resolveDockerManifestBindMounts(manifest) {
    return await Promise.all(manifest
        .mountTargetsForMaterialization()
        .filter(({ entry }) => isDockerBindMount(entry))
        .map(async ({ mountPath, entry }) => await resolveDockerMountAuthority(entry.source, mountPath, entry.readOnly ?? true)));
}
function dockerMountAuthorityFingerprint(sessionIdentity, workspaceMount, pathGrantMounts, manifestBindMounts, manifest) {
    const mountedGrants = [...pathGrantMounts].sort((left, right) => left.target < right.target ? -1 : left.target > right.target ? 1 : 0);
    const mountedManifestBinds = [...manifestBindMounts].sort((left, right) => left.target < right.target ? -1 : left.target > right.target ? 1 : 0);
    return (0, node_crypto_1.createHash)('sha256')
        .update((0, stableJson_1.stableJsonStringify)({
        sessionIdentity,
        workspaceMount,
        mountedGrants,
...
function dockerMaterializedEntriesFingerprint(manifest) {
    return (0, node_crypto_1.createHash)('sha256')
        .update((0, stableJson_1.stableJsonStringify)({
...
function dockerNonMountManifestEntries(manifest) {
    const flattened = [];
    const visit = (children, parentPath = '') => {
        for (const [name, entry] of Object.entries(children).sort(([left], [right]) => left.localeCompare(right))) {
            ...
            if (entry.type === 'dir') {
                ...
                flattened.push({ path, entry: directory });
                if (nestedChildren) {
...
function dockerExposedPortSetsEqual(left, right) {
    return (JSON.stringify([...left].sort((a, b) => a - b)) ===
        JSON.stringify([...right].sort((a, b) => a - b)));
}
function normalizeDockerLabels(value, source) {
    if (value === undefined) {
        return {};
    }
    if (!(0, typeGuards_1.isStringRecord)(value)) {
        throw new errors_1.UserError(`${source} must be a record of string values.`);
    }
    const labels = { ...value };
    for (const key of Object.keys(labels)) {
        if (RESERVED_DOCKER_LABELS.has(key)) {
            throw new errors_1.UserError(`${source} cannot override reserved SDK label "${key}".`);
        }
    }
    return labels;
}
function dockerLabelRecordsEqual(left, right) {
    const leftKeys = Object.keys(left);
    return (leftKeys.length === Object.keys(right).length &&
        leftKeys.every((key) => right[key] === left[key]));
}
function resolveDockerLabels(clientOptions, perRunOptions, explicitStateFallback) {
    const trustedLabels = perRunOptions?.labels !== undefined
        ? normalizeDockerLabels(perRunOptions.labels, 'DockerSandboxClient per-run labels')
        : clientOptions.labels !== undefined
            ? normalizeDockerLabels(clientOptions.labels, 'DockerSandboxClient labels')
            : undefined;
    const persistedLabels = explicitStateFallback
        ? normalizeDockerLabels(explicitStateFallback.labels, 'Docker sandbox session state labels')
        : undefined;
    if (trustedLabels !== undefined &&
        persistedLabels !== undefined &&
        !dockerLabelRecordsEqual(trustedLabels, persistedLabels)) {
        throw new errors_1.UserError('DockerSandboxClient labels cannot be changed when resuming explicit session state. Start a fresh sandbox session instead.');
    }
    return { ...(trustedLabels ?? persistedLabels ?? {}) };
}
function dockerRequiredLabelsMatch(requiredLabels, actualLabels) {
    const normalizedRequiredLabels = normalizeDockerLabels(requiredLabels, 'Docker sandbox session state labels');
    return Object.entries(normalizedRequiredLabels).every(([key, value]) => actualLabels?.[key] === value);
}
function validateDockerNetworkConfiguration(networkMode, exposedPorts) {
    if (networkMode !== undefined && networkMode !== 'none') {
        throw new errors_1.UserError('DockerSandboxClient networkMode must be "none" when provided.');
    }
    if (networkMode === 'none' && (exposedPorts?.length ?? 0) > 0) {
        throw new errors_1.UserError('DockerSandboxClient exposedPorts cannot be used when networkMode is "none".');
    }
    return networkMode;
}
function resolveDockerNetworkConfiguration(clientOptions, perRunOptions, explicitStateFallback) {
    const trustedExposedPorts = perRunOptions?.exposedPorts !== undefined
        ? (0, sessionStateValues_1.normalizeExposedPorts)(perRunOptions.exposedPorts)
        : clientOptions.exposedPorts !== undefined
            ? (0, sessionStateValues_1.normalizeExposedPorts)(clientOptions.exposedPorts)
            : undefined;
    const exposedPorts = explicitStateFallback
        ? (0, sessionStateValues_1.normalizeExposedPorts)(explicitStateFallback.configuredExposedPorts)
        : (trustedExposedPorts ?? []);
    if (explicitStateFallback &&
        trustedExposedPorts !== undefined &&
        !dockerExposedPortSetsEqual(trustedExposedPorts, exposedPorts)) {
        throw new errors_1.UserError('DockerSandboxClient exposedPorts cannot be changed when resuming explicit session state. Start a fresh sandbox session instead.');
    }
    const networkMode = validateDockerNetworkConfiguration(perRunOptions?.networkMode !== undefined
        ? perRunOptions.networkMode
        : clientOptions.networkMode !== undefined
            ? clientOptions.networkMode
            : explicitStateFallback?.networkMode, exposedPorts);
    return { configuredExposedPorts: exposedPorts, networkMode };
}
function assertDockerManifestDeltaSupported(currentManifest, deltaManifest) {
    const currentGrantsByPath = new Map(currentManifest.extraPathGrants.map((grant) => [grant.path, grant]));
    const splitGrant = deltaManifest.extraPathGrants.find((grant) => grant.hostPath !== undefined ||
        currentGrantsByPath.get(grant.path)?.hostPath !== undefined);
    if (splitGrant) {
        throw new errors_2.SandboxUnsupportedFeatureError(`DockerSandboxClient cannot add or change path grant hostPath for "${splitGrant.path}" on an already-running container. Configure it in the manifest passed to create().`, {
            provider: 'DockerSandboxClient',
            feature: 'manifest.extraPathGrants.hostPath',
            path: splitGrant.path,
        });
    }
    (0, localWorkspace_1.assertLocalWorkspaceManifestMetadataSupported)('DockerSandboxClient', deltaManifest, {
        allowLocalBindMounts: false,
        allowIdentityMetadata: true,
        supportsMount: isSupportedDockerApplyMount,
    });
...
function stripDockerIdentityMetadata(manifest) {
    const stripped = new manifest_1.Manifest({
    ...
    (0, manifest_1.copyManifestMountCredentialExposurePolicy)(stripped, manifest);
    return stripped;
}
...
async function inspectContainerRunning(containerId) {
    ...
    if (result.status !== 0) {
        ...
        throw new errors_1.UserError(`Failed to inspect Docker sandbox container: ${(0, runProcess_1.formatSandboxProcessError)(result)}`);
    }
    ...
    throw new errors_1.UserError(`Failed to inspect Docker sandbox container: unexpected running state "${running}".`);
}
...
async function startDockerContainer(args) {
    const labels = normalizeDockerLabels(args.labels, 'Docker sandbox container labels');
    ...
    const workspaceMount = await resolveDockerWorkspaceMount(args.workspaceRootPath, args.manifestRoot);
    const pathGrantMounts = await resolveDockerPathGrantMounts(args.manifest);
    const manifestBindMounts = await resolveDockerManifestBindMounts(args.manifest);
    const { mountArgs, volumeNames } = dockerMountArgsForManifest(args.manifest, containerName, manifestBindMounts);
...
function dockerSplitPathGrantContainingPath(manifest, path) {
    const { grant } = new workspacePaths_1.WorkspacePathPolicy({
        root: manifest.root,
        extraPathGrants: manifest.extraPathGrants,
    }).resolve(path);
...
function sandboxPathGrantUsesDistinctHostPath(grant) {
...
async function applyDockerInContainerMounts(session, manifest, preparedMounts) {
    const mounts = preparedMounts ?? (await prepareDockerInContainerMounts(session, manifest));
    const appliedMounts = [];
    for (const preparedMount of mounts) {
        try {
            const mountPath = await applyPreparedDockerInContainerMount(session, preparedMount);
            appliedMounts.push(dockerAppliedMountCleanupTarget(preparedMount, mountPath));
        }
        catch (error) {
...
async function prepareDockerInContainerMounts(session, manifest, credentialBoundaryManifest = manifest, environment = session.state.environment) {
    const preparedMounts = [];
    for (const { logicalPath, entry, } of manifest.mountTargetsForMaterialization()) {
        if (isDockerInContainerMount(entry)) {
            preparedMounts.push(await prepareDockerInContainerMount(session, logicalPath, entry, manifest, credentialBoundaryManifest, environment));
        }
    }
    return preparedMounts;
}
async function prepareDockerInContainerMount(session, logicalPath, entry, manifest, credentialBoundaryManifest = manifest, environment = session.state.environment) {
    const mountPath = resolveDockerMountPath(session.state.manifest.root, logicalPath, entry);
    const prepareCandidate = async () => {
        const effectiveMountPath = await resolveDockerEffectiveMountPath(session, logicalPath, entry, mountPath);
        const credentialExposure = (0, mountSecurity_1.validateMountCredentialBoundariesAtEffectivePath)(manifest, logicalPath, entry, effectiveMountPath);
        const preparedCredentialFiles = await prepareDockerMountCredentialFiles(session, credentialBoundaryManifest, entry, mountPath, environment);
        return {
            entry: preparedCredentialFiles.entry,
            effectiveMountPath,
            pattern: dockerInContainerMountPattern(preparedCredentialFiles.entry),
            credentialExposureAcknowledged: credentialExposure.credentialExposureAcknowledged,
            allowAmbientCredentials: credentialExposure.broadCredentialExposureAcknowledged,
    ...
    return {
        ...
        revalidateMountAuthority: async () => {
            ...
            if (current.effectiveMountPath !== candidate.effectiveMountPath ||
                ...
                (0, stableJson_1.stableJsonStringify)(current.entry) !==
                    (0, stableJson_1.stableJsonStringify)(candidate.entry) ||
                (0, stableJson_1.stableJsonStringify)(current.environment) !==
                    (0, stableJson_1.stableJsonStringify)(candidate.environment) ||
                (0, stableJson_1.stableJsonStringify)(current.pattern) !==
                    (0, stableJson_1.stableJsonStringify)(candidate.pattern)) {
                throw new errors_2.SandboxMountError('Docker sandbox mount authority changed after validation. Retry from current trusted configuration.', { mountType: entry.type }, 'mount_config_invalid');
            }
        },
    };
}
async function prepareDockerMountCredentialFiles(session, manifest, entry, mountPath, environment) {
    const references = (0, mountSecurity_1.mountCredentialFileReferences)(manifest, entry, environment);
    if (references.length === 0) {
        return { entry: structuredClone(entry), environment };
    }
    const resolveEffectivePath = async (path) => {
        const candidate = path.startsWith('/')
            ? (0, posixPath_1.normalizePosixPath)(path)
            : (0, localWorkspace_1.joinSandboxLogicalPath)(manifest.root, path);
        const resolvedPath = (await resolveTrustedDockerPath(session, candidate, 'resolve Docker mount credential path')).trim();
        if (!resolvedPath.startsWith('/') || resolvedPath.includes('\n')) {
            throw new errors_2.SandboxMountError('DockerSandboxClient failed to resolve an absolute mount credential path.', { mountType: entry.type }, 'mount_config_invalid');
        }
        return (0, posixPath_1.normalizePosixPath)(resolvedPath);
    };
    const manifestEntries = await Promise.all([...manifest.iterEntries()]
        .filter(({ entry }) => !(0, entries_1.isMount)(entry))
    ...
    const resolvedReferences = await Promise.all(references.map(async ({ field, path }) => {
        if (field === 'mountStrategy.pattern.configFilePath') {
            try {
                resolveDockerRcloneConfigPath(manifest, path);
            }
            catch {
                throw new errors_2.SandboxMountError('DockerSandboxClient failed to resolve the rclone config file.', { mountType: entry.type }, 'mount_config_invalid');
            }
        }
        return { field, path: await resolveEffectivePath(path) };
    }));
    (0, mountSecurity_1.validateMountCredentialFileEffectivePaths)({
        entry,
        mountPath,
        credentialFiles: resolvedReferences,
        manifestEntries,
    });
    const preparedEntry = structuredClone(entry);
...
async function resolveTrustedDockerPath(session, path, action) {
    const dockerArgs = ['exec', '-i', '-w', '/'];
    ...
    dockerArgs.push('-e', 'PATH=/usr/bin:/bin', '-e', 'HOME=/root', '-u', 'root', session.state.containerId, '/usr/bin/realpath', '-m', '--', path);
    let result;
    try {
...
function writeDockerNestedField(record, field, value) {
...
async function applyPreparedDockerInContainerMount(session, prepared) {
    const { entry, effectiveMountPath, pattern, allowAmbientCredentials, environment, } = prepared;
    await prepared.revalidateMountAuthority();
    try {
        switch (pattern.type) {
            case 'rclone':
                await applyDockerRcloneMount(session, entry, effectiveMountPath, pattern, allowAmbientCredentials, environment, prepared.revalidateMountAuthority);
                return effectiveMountPath;
            case 'mountpoint':
                await applyDockerMountpointMount(session, entry, effectiveMountPath, pattern, allowAmbientCredentials, environment, prepared.revalidateMountAuthority);
                return effectiveMountPath;
            case 's3files':
                await applyDockerS3FilesMount(session, entry, effectiveMountPath, pattern, allowAmbientCredentials, prepared.revalidateMountAuthority);
                return effectiveMountPath;
            case 'fuse':
                await applyDockerFuseMount(session, entry, effectiveMountPath, pattern, allowAmbientCredentials, prepared.revalidateMountAuthority);
                return effectiveMountPath;
            default:
        ...
        return await throwAfterDockerAppliedMountCleanup(session, [dockerAppliedMountCleanupTarget(prepared, effectiveMountPath)], error);
    }
}
async function resolveDockerEffectiveMountPath(session, logicalPath, entry, declaredMountPath) {
    const workspaceRelativePath = resolveDockerMountWorkspaceRelativePath(session.state.manifest.root, logicalPath, entry);
    if (workspaceRelativePath === null) {
        const resolvedPath = (await resolveTrustedDockerPath(session, declaredMountPath, 'resolve Docker mount path')).trim();
        if (!resolvedPath.startsWith('/') || resolvedPath.includes('\n')) {
            throw new errors_2.SandboxMountError('DockerSandboxClient failed to resolve an absolute mount path.', { mountType: entry.type }, 'mount_config_invalid');
        }
        return (0, posixPath_1.normalizePosixPath)(resolvedPath);
    }
    const workspaceRoot = await (0, promises_1.realpath)(session.state.workspaceRootPath);
    const effectiveHostPath = await realpathWithMissingTail((0, node_path_1.resolve)(session.state.workspaceRootPath, workspaceRelativePath));
    const effectiveWorkspacePath = (0, node_path_1.relative)(workspaceRoot, effectiveHostPath);
    if (effectiveWorkspacePath === '..' ||
        effectiveWorkspacePath.startsWith(`..${node_path_1.sep}`) ||
        (0, node_path_1.isAbsolute)(effectiveWorkspacePath)) {
        throw new errors_2.SandboxMountError('DockerSandboxClient mount path resolves outside the sandbox workspace.', { mountType: entry.type }, 'mount_config_invalid');
    }
    return (0, localWorkspace_1.joinSandboxLogicalPath)(session.state.manifest.root, effectiveWorkspacePath.split(node_path_1.sep).join('/'));
}
async function realpathWithMissingTail(path) {
    ...
    while (true) {
        try {
            return (0, node_path_1.resolve)(await (0, promises_1.realpath)(existingPath), ...missingSegments);
        }
        catch (error) {
            if (!isMissingPathError(error)) {
                throw error;
            }
            const parent = (0, node_path_1.dirname)(existingPath);
            if (parent === existingPath) {
                throw error;
            }
            missingSegments.unshift((0, node_path_1.basename)(existingPath));
            existingPath = parent;
        }
    }
}
...
function dockerAppliedMountCleanupTarget(prepared, mountPath) {
    return {
        mountPath,
        verifyRcloneNfsTermination: prepared.pattern.type === 'rclone' &&
            prepared.pattern.mode === 'nfs',
    };
}
async function cleanupDockerAppliedMounts(session, targets) {
    const cleanupErrors = [];
    ...
    if (cleanupErrors.length > 0) {
        throw new errors_2.SandboxMountError('DockerSandboxClient credential-bearing mount cleanup failed.', {
            provider: 'docker',
...
async function throwAfterDockerAppliedMountCleanup(session, targets, mountError) {
    try {
        await cleanupDockerAppliedMounts(session, targets);
    }
    catch {
        throw new errors_2.SandboxMountError('DockerSandboxClient mount failed and credential cleanup could not complete.', {
            provider: 'docker',
...
async function applyDockerRcloneMount(session, entry, mountPath, pattern, allowAmbientCredentials, environment, revalidateMountAuthority) {
    ...
    const config = await dockerRcloneMountConfig(session, entry, pattern, mountPath, allowAmbientCredentials);
    ...
    if (mode === 'nfs') {
        ...
        ].join(' && '), `mount rclone ${entry.type}`, { input: config.configText, environment });
        return;
    }
    const mountArgs = [
        'rclone',
...
async function applyDockerMountpointMount(session, entry, mountPath, pattern, allowAmbientCredentials, environment, revalidateMountAuthority) {
    ...
    const hasExplicitCredentials = entry.type === 's3_mount'
        ? Boolean(entry.accessKeyId && entry.secretAccessKey)
        : Boolean((0, typeGuards_1.readOptionalString)(entry, 'accessId') &&
            (0, typeGuards_1.readOptionalString)(entry, 'secretAccessKey'));
    const mountArgs = [
        'mount-s3',
    ...
    const cleanupEnvCommand = `rm -rf -- ${(0, shell_1.shellQuote)(envDir)}`;
    ...
    const clearInheritedCredentialAuthorityCommand = clearedInheritedCredentialAuthorityNames.join(' ');
    const command = envText
        ? [
            `trap ${(0, shell_1.shellQuote)(cleanupEnvCommand)} EXIT HUP INT TERM`,
            cleanupEnvCommand,
        ...
        : shellCommand(mountArgs);
    await revalidateMountAuthority();
    await session.runDockerMountCommand([`mkdir -p -- ${(0, shell_1.shellQuote)(mountPath)}`, command].join(' && '), `mount mountpoint ${entry.type}`, envText
        ? {
            input: envText,
            environment: Object.fromEntries([
                ...Object.entries(environment),
                ...clearedInheritedCredentialAuthorityNames.map((name) => [name, '']),
            ]),
        }
        : { environment });
}
...
async function applyDockerS3FilesMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority) {
    if (!allowAmbientCredentials) {
        throw new errors_2.SandboxMountError('DockerSandboxClient s3files mounts require explicit trusted credential exposure for their workload identity provider.', { mountPath, mountType: entry.type }, 'mount_config_invalid');
    }
    if (entry.type !== 's3_files_mount') {
...
async function applyDockerFuseMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority) {
    if (entry.type === 'azure_blob_mount') {
        await applyDockerAzureBlobFuseMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority);
        return;
    }
    if (!pattern.command) {
...
async function applyDockerAzureBlobFuseMount(session, entry, mountPath, pattern, allowAmbientCredentials, revalidateMountAuthority) {
    ...
    if (!entry.accountKey &&
        ...
        throw new errors_2.SandboxMountError('DockerSandboxClient blobfuse mounts require explicit trusted credential exposure before using managed identity.', { mountPath, mountType: entry.type }, 'mount_config_invalid');
    }
...
function dockerMountArgsForManifest(manifest, containerName, manifestBindMounts) {
    const mountArgs = [];
    const volumeNames = [];
    const bindMountsByTarget = new Map(manifestBindMounts.map((mount) => [mount.target, mount]));
    for (const { mountPath, entry, } of manifest.mountTargetsForMaterialization()) {
        if (isDockerBindMount(entry)) {
            ...
            if (!bindMount) {
                throw new errors_2.SandboxConfigurationError(`Docker bind mount authority was not resolved for "${mountPath}".`);
            }
...
function dockerExtraPathGrantMountArgs(pathGrantMounts) {
...
async function dockerRcloneMountConfig(session, entry, pattern, mountPath, allowAmbientCredentials) {
    ...
    const withConfigText = async (config) => {
        ...
        let sourcePath;
        try {
            sourcePath = resolveDockerRcloneConfigPath(session.state.manifest, configFilePath);
        }
        catch {
            throw new errors_2.SandboxMountError('DockerSandboxClient failed to resolve the rclone config file.', { mountType: entry.type }, 'mount_config_invalid');
        }
        const encodedConfig = await session.runDockerMountCommand(`base64 -- ${(0, shell_1.shellQuote)(sourcePath)}`, 'read rclone config');
    ...
    switch (entry.type) {
        ...
        case 'gcs_mount':
            return await withConfigText(dockerGcsRcloneMountConfig(entry, remoteName, allowAmbientCredentials));
        case 'azure_blob_mount':
            return await withConfigText(dockerAzureBlobRcloneMountConfig(entry, remoteName, allowAmbientCredentials));
        case 'box_mount':
            return await withConfigText({
                remoteName,
...
function dockerAzureBlobRcloneMountConfig(entry, remoteName, allowAmbientCredentials) {
    const account = entry.account ?? entry.accountName;
    if (!account) {
...
function dockerGcsRcloneMountConfig(entry, remoteName, allowAmbientCredentials) {
...
function s3CredentialLines(entry, allowAmbientCredentials) {
    const lines = [];
...
function validateCredentialPair(provider, mountType, accessKeyId, secretAccessKey) {
    (0, credentials_1.validateCredentialPair)({
        accessKeyId,
        secretAccessKey,
