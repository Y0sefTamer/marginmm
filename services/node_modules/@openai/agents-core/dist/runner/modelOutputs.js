...
const toolCaller_1 = require("./toolCaller.js");
const items_2 = require("./items.js");
const toolSearchAttribution_1 = require("./toolSearchAttribution.js");
...
function ensureHostedToolCallAllowed(output, tools, mcpToolMap, loadedToolNames, agent) {
    ...
    if (isCodeInterpreterCall) {
        ...
        (0, toolCaller_1.ensureToolCallerAllowed)(output, codeInterpreterTool.providerData?.allowed_callers, codeInterpreterTool.name, agent);
        return;
    }
    ...
    (0, toolCaller_1.ensureToolCallerAllowed)(output, mcpTool.providerData.allowed_callers, serverLabel, agent);
...
function handleToolCallAction({ output, tool, agent, errorMessage, errorData, items, toolsUsed, actions, buildAction, }) {
    ...
    (0, toolCaller_1.ensureToolCallerAllowed)(output, resolvedTool.allowedCallers, resolvedTool.name, agent);
...
function resolveFunctionOrHandoff(toolCall, handoffMap, functionMap) {
    ...
    const functionTool = (0, toolIdentity_1.resolveFunctionToolCall)(toolCall, functionMap);
    if (!functionTool) {
        return {
            type: 'not_found',
            toolName: (0, toolIdentity_1.getToolCallNamespace)(toolCall) &&
                (0, toolIdentity_1.getToolCallNamespace)(toolCall) !== toolCall.name
                ? `${(0, toolIdentity_1.getToolCallNamespace)(toolCall)}.${toolCall.name}`
                : toolCall.name,
        };
    }
...
function collectLoadedDeferredToolStateFromHistory(items, agent, agentName) {
    const state = {
    ...
    for (const item of (0, items_2.trimToLatestCompaction)(items)) {
        if ((item instanceof items_1.RunToolSearchOutputItem ||
            item instanceof items_1.RunInputItem) &&
            item.agent !== agent) {
            continue;
        }
...
function buildFunctionToolMap(tools) {
    return (0, toolIdentity_1.buildFunctionToolLookupMap)(tools.filter((t) => t.type === 'function'));
}
function getUnresolvedClientToolSearchCalls(modelResponse, hasClientToolSearchTool) {
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'tool_search_call') {
            const execution = (0, utils_1.getToolSearchExecution)(output);
            if (execution === 'client' ||
        ...
        if (output.type !== 'tool_search_output' ||
            (0, utils_1.getToolSearchExecution)(output) === 'server') {
            continue;
        }
...
async function buildGeneratedClientToolSearchOutputMapAsync(args) {
    ...
    for (const toolSearchCall of clientToolSearchCalls) {
        if (clientToolSearchTool &&
            ...
            (0, toolSearch_1.registerRuntimeToolSearchTools)({
 ...
 */
function processModelResponse(modelResponse, agent, tools, handoffs, priorItems = [], toolNotFoundBehavior = 'raise_error', processingOptions = {}) {
    (0, items_2.assertValidCompactionItems)(modelResponse.output);
    const items = [];
    const runHandoffs = [];
    const runFunctions = [];
    ...
    const loadedDeferredToolState = collectLoadedDeferredToolStateFromHistory(priorItems, agent, processingOptions.toolSearchAgentName);
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'message') {
            ...
            if (generatedOutput) {
                items.push(new items_1.RunToolSearchOutputItem((0, toolSearchAttribution_1.attributeToolSearchOutput)(generatedOutput, processingOptions.toolSearchAgentName), agent));
            ...
            items.push(new items_1.RunToolSearchOutputItem((0, toolSearchAttribution_1.attributeToolSearchOutput)(output, processingOptions.toolSearchAgentName), agent));
            ...
            items.push(new items_1.RunCompactionItem(output, agent));
        }
            ...
            (0, toolCaller_1.ensureToolCallerAllowed)(output, resolvedShellTool.allowedCallers, resolvedShellTool.name, agent);
            items.push(new items_1.RunToolCallItem(output, agent));
        ...
        if (resolved.type === 'not_found') {
            ...
            (0, toolCaller_1.ensureToolCallerAllowed)(output, undefined, resolved.handoff.toolName, agent);
            recordHandoffRequest(output, resolved.handoff, agent, items, toolsUsed, runHandoffs);
        }
        else {
            (0, toolCaller_1.ensureToolCallerAllowed)(output, resolved.tool.allowedCallers, (0, toolIdentity_1.getFunctionToolQualifiedName)(resolved.tool) ?? resolved.tool.name, agent);
            ...
            items.push(new items_1.RunToolCallItem(normalizedToolCall, agent, !(0, toolIdentity_1.getToolCallNamespace)(normalizedToolCall) &&
                (0, toolIdentity_1.isDeferredTopLevelFunctionTool)(resolved.tool)
                ? (0, toolIdentity_1.getFunctionToolStateKey)(resolved.tool)
                : undefined));
            runFunctions.push((0, types_1.createToolRunFunction)({
...
async function processModelResponseAsync(modelResponse, agent, tools, handoffs, state, priorItems = [], toolNotFoundBehavior = 'raise_error', processingOptions = {}) {
    (0, items_2.assertValidCompactionItems)(modelResponse.output);
    processingOptions = {
        ...processingOptions,
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'message') {
            ...
            const generatedOutput = generatedClientToolSearchOutputsByCall.get(output);
            if (generatedOutput) {
                items.push(new items_1.RunToolSearchOutputItem((0, toolSearchAttribution_1.attributeToolSearchOutput)(generatedOutput.output, processingOptions.toolSearchAgentName), agent));
                ...
                const registeredRuntimeTools = (0, toolSearch_1.registerRuntimeToolSearchTools)({
                    availableTools,
                    functionMap,
                    handoffMap,
                    mcpToolMap,
