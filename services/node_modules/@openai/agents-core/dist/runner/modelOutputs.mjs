...
import { RunCompactionItem, RunHandoffCallItem, RunInputItem, RunMessageOutputItem, RunReasoningItem, RunToolApprovalItem, RunToolCallItem, RunToolCallOutputItem, RunToolSearchCallItem, RunToolSearchOutputItem, } from "../items.mjs";
...
import { buildFunctionToolLookupMap, getFunctionToolNamespace, getFunctionToolQualifiedName, getFunctionToolStateKey, isDeferredTopLevelFunctionTool, getToolCallNamespace, resolveFunctionToolCall, } from "../toolIdentity.mjs";
...
import { assertValidCompactionItems, trimToLatestCompaction } from "./items.mjs";
import { attributeToolSearchOutput } from "./toolSearchAttribution.mjs";
...
export function processModelResponse(modelResponse, agent, tools, handoffs, priorItems = [], toolNotFoundBehavior = 'raise_error', processingOptions = {}) {
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'message') {
            ...
            if (generatedOutput) {
                items.push(new RunToolSearchOutputItem(attributeToolSearchOutput(generatedOutput, processingOptions.toolSearchAgentName), agent));
            ...
            items.push(new RunToolSearchOutputItem(attributeToolSearchOutput(output, processingOptions.toolSearchAgentName), agent));
        ...
        if (resolved.type === 'not_found') {
            ...
            items.push(new RunToolCallItem(normalizedToolCall, agent, !getToolCallNamespace(normalizedToolCall) &&
...
export async function processModelResponseAsync(modelResponse, agent, tools, handoffs, state, priorItems = [], toolNotFoundBehavior = 'raise_error', processingOptions = {}) {
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'message') {
            ...
            const generatedOutput = generatedClientToolSearchOutputsByCall.get(output);
            if (generatedOutput) {
                items.push(new RunToolSearchOutputItem(attributeToolSearchOutput(generatedOutput.output, processingOptions.toolSearchAgentName), agent));
