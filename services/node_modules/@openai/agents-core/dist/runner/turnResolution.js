 ...
 */
async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, validateHandoffAgent, beforeHandoffInputFilter) {
...
async function resolveHandoffAfterTools(options) {
    const { agent, originalInput, preStepItems, newItems, newResponse, handoffRuns, functionResults, additionalInterruptions, runner, state, signal, validateHandoffAgent, beforeHandoffInputFilter, } = options;
    if (handoffRuns.length === 0) {
        return undefined;
    }
    if (signal?.aborted) {
        ...
        for (const { toolCall } of handoffRuns) {
            ...
            appendRunItemIfNew(new items_1.RunToolCallOutputItem(rawItem, agent, rawItem.output), newItems, appendContext);
        }
        return undefined;
    }
    const interruptions = (0, toolExecution_1.collectInterruptions)(functionResults, additionalInterruptions);
    ...
    return (0, toolExecution_1.executeHandoffCalls)(agent, originalInput, preStepItems, newItems, newResponse, handoffRuns, runner, state._context, (0, invocationContext_1.getRunStateTurnSpanParent)(state) ?? state._currentAgentSpan, validateHandoffAgent, beforeHandoffInputFilter);
}
