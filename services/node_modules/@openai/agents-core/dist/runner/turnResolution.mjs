...
import { RunHandoffCallItem, RunMessageOutputItem, RunToolApprovalItem, RunToolCallItem, RunToolCallOutputItem, } from "../items.mjs";
import logger, { logToolActionWarning } from "../logger.mjs";
import { getOutputText, getRefusalFromOutputMessage, getTextFromOutputMessage, } from "../utils/messages.mjs";
import { safeExecute } from "../utils/safeExecute.mjs";
import { isDataRedactedError, processFinalOutputWithRedaction, REDACTED_FINAL_OUTPUT_ERROR_MESSAGE, } from "../utils/finalOutputError.mjs";
...
import { buildFunctionToolLookupMap, getFunctionToolStateKey, getFunctionToolStateKeyForCall, getFunctionToolStateKeyForResolvedCall, getFunctionToolStateKeys, getToolCallNamespace, resolveFunctionToolCall, getHostedMcpApprovalRequestIdentity, getHostedMcpApprovalRequestKey, } from "../toolIdentity.mjs";
import { createRunErrorFinalOutputItem, formatRunErrorFinalOutput, invalidateAcceptedResponseReplayEvidence, preserveInvalidFinalOutputRedaction, resolveRunErrorHandler, validateRunErrorHandlerFinalOutput, } from "./errorHandlers.mjs";
import { getRunOutput, getTurnInput } from "./items.mjs";
import { buildApplyPatchAbortResult, buildFunctionAbortResult, buildShellAbortResult, } from "./streamReconciliation.mjs";
import { runWithSiblingCancellation } from "./siblingCancellation.mjs";
import { getHandoffToolInvocationName, getHostedMcpApprovalToolName, getToolInvocationCallId, getToolInvocationFingerprint, } from "../toolInvocation.mjs";
const DEFAULT_TOOL_NOT_FOUND_MESSAGE = (toolName) => `Tool '${toolName}' not found.`;
export function preflightModelResponseToolInvocations(agent, state, modelResponse, tools, handoffs) {
    const functionMap = buildFunctionToolLookupMap(tools.filter((tool) => tool.type === 'function'));
    ...
    const validate = (toolName, rawItem) => {
        ...
        if (previous !== undefined && previous !== fingerprint) {
            throw new ModelBehaviorError(`Tool call ID ${callId} was reused for different tool invocations in the same model response.`, state);
        }
    ...
    for (const output of modelResponse.output) {
        if (output.type === 'function_call') {
            ...
            validate(resolvedTool
                ...
                : getFunctionToolStateKeyForCall(output, output.name), output);
            continue;
        }
...
export function preflightToolInvocations(agent, state, processedResponse) {
    ...
    for (const run of processedResponse.functions ?? []) {
        observe(getFunctionToolStateKey(run.tool) ??
            getFunctionToolStateKeyForCall(run.toolCall, run.tool.name), run.toolCall);
    }
    for (const run of processedResponse.functionToolsNotFound ?? []) {
        observe(getFunctionToolStateKeyForCall(run.toolCall, run.toolName), run.toolCall);
    }
    ...
    for (const run of processedResponse.mcpApprovalRequests ?? []) {
        ...
        if (sourceRawItems.length !== 1) {
            throw new ModelBehaviorError(`Hosted MCP approval request ${observation.callId} does not have exactly one source tool invocation.`);
        }
        const sourceRawItem = sourceRawItems[0];
        claimedHostedMcpItems.add(sourceRawItem);
        if (getToolInvocationFingerprint(toolName, sourceRawItem) !==
            observation.fingerprint) {
            throw new ModelBehaviorError(`Hosted MCP approval request ${observation.callId} does not match its source tool invocation.`);
        }
...
export function filterSuppressedToolCallItems(items, suppressed) {
...
async function resolveToolNotFoundMessage(state, toolRun, toolErrorFormatter) {
    ...
    try {
        ...
        logToolActionWarning(logger, 'toolErrorFormatter threw while formatting tool not found:', error);
    }
    return defaultMessage;
}
...
async function resolveInvalidFinalOutput(args) {
    return preserveInvalidFinalOutputRedaction(async (redactFromStart) => {
        ...
        const outputText = formatRunErrorFinalOutput(args.agent, handlerResult.finalOutput);
        validateRunErrorHandlerFinalOutput(args.agent, outputText, true, redactFromStart);
 ...
 */
export async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, validateHandoffAgent, beforeHandoffInputFilter) {
    ...
    const functionToolRuns = processedResponse.functions.filter((run) => {
        ...
        const isPendingNested = getFunctionToolStateKeys(run.tool, run.availableFunctionTools ?? [run.tool]).some((stateKey) => state.hasPendingAgentToolRun(stateKey, callId));
    ...
    const functionResults = await executeFunctionToolCalls(agent, functionToolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal);
    ...
    const computerResults = pendingComputerActions.length > 0
        ? await executeComputerActions(agent, pendingComputerActions, runner, state._context, undefined, toolErrorFormatter, signal)
        : [];
    ...
    const hostedMcpApprovals = await handleHostedMcpApprovals({
        ...
        resolveApproval: (rawItem) => {
            ...
            return state._context._resolveToolInvocationApproval(agent, getHostedMcpApprovalToolName(rawItem.name, rawItem), rawItem);
        },
        resolveRejectionMessage: (rawItem) => state._context._getToolInvocationRejectionMessage(agent, getHostedMcpApprovalToolName(rawItem.name, rawItem), rawItem),
    });
    ...
    const preStepItems = originalPreStepItems.filter((item) => {
        ...
        if (isHostedMcpApprovalItem(item)) {
            ...
            const approvalIdentity = getHostedMcpApprovalRequestIdentity(item.rawItem);
 ...
 */
export async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers, signal, preflightedToolCalls, attemptedErrors, validateHandoffAgent) {
    ...
    const runFunctionTools = (executionSignal = signal, cancelSiblingCategories) => executeFunctionToolCalls(agent, functionRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig, executionSignal, cancelSiblingCategories);
    const runComputerActions = (executionSignal = signal, cancelSiblingCategories) => executeComputerActions(agent, processedResponse.computerActions.filter((run) => !suppressedToolCalls.has(run.toolCall)), runner, state._context, undefined, toolErrorFormatter, executionSignal, cancelSiblingCategories);
    const [functionResults, computerResults] = processedResponse.functions.some((run) => !suppressedToolCalls.has(run.toolCall)) &&
        ...
        ? await runWithSiblingCancellation([runFunctionTools, runComputerActions], signal)
    ...
    if (!hasPendingToolsOrApprovals && messageItems.length > 0) {
        ...
        if (refusal && typeof potentialFinalOutput === 'undefined') {
            ...
            return new SingleStepResult(originalInput, newResponse, preStepItems, newItems, { type: 'next_step_final_output', output: outputText }, 'error_handler');
        }
    }
    ...
    if (typeof potentialFinalOutput === 'undefined' ||
        ...
        if (!hasPendingToolsOrApprovals && isMissingStructuredFinalOutput) {
            const outputError = new ModelBehaviorError('Model returned no final output for the structured output type.');
            ...
            if (typeof handledOutput !== 'undefined') {
                return new SingleStepResult(originalInput, newResponse, preStepItems, newItems, { type: 'next_step_final_output', output: handledOutput }, 'error_handler');
            }
        }
        return new SingleStepResult(originalInput, newResponse, preStepItems, newItems, { type: 'next_step_run_again' });
    }
    if (!hasPendingToolsOrApprovals) {
        ...
        if (agent.outputType !== 'text' && potentialFinalOutput) {
            ...
            const [error] = await safeExecute(() => processFinalOutputWithRedaction(() => agent.processFinalOutput(potentialFinalOutput)));
            if (error) {
...
async function resolveHandoffAfterTools(options) {
    ...
    if (signal?.aborted) {
        ...
        for (const { toolCall } of handoffRuns) {
            ...
            appendRunItemIfNew(new RunToolCallOutputItem(rawItem, agent, rawItem.output), newItems, appendContext);
        }
        return undefined;
    }
    const interruptions = collectInterruptions(functionResults, additionalInterruptions);
    ...
    return executeHandoffCalls(agent, originalInput, preStepItems, newItems, newResponse, handoffRuns, runner, state._context, getRunStateTurnSpanParent(state) ?? state._currentAgentSpan, validateHandoffAgent, beforeHandoffInputFilter);
}
