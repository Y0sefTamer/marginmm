...
export const MAX_HBAR_PRICE_TINYBAR = 10000000n;
...
export function assertHederaFacilitatorSupport(supported) {
...
export function buildCalibrationRoutes(config) {
...
export async function createCalibrationPaymentMiddleware(config) {
    const facilitator = new ValidatingBlockyFacilitatorClient({
        url: BLOCKY_TESTNET_FACILITATOR_URL,
        timeoutMs: 15_000,
    });
    const resourceServer = new x402ResourceServer(facilitator).register(HEDERA_TESTNET, new ExactHederaServerScheme({}));
    await resourceServer.initialize();
    if (!facilitator.capability)
        throw new Error("Facilitator capability validation did not run");
    return {
...
export function buildStrictHbarPaymentPolicy(config) {
    assertHederaAccountId(config.serviceAccountId, "serviceAccountId");
    parseHbarPriceTinybar(config.priceTinybar);
    return (x402Version, requirements) => {
        if (x402Version !== 2)
            return [];
        return requirements.filter((requirement) => (requirement.scheme === "exact"
...
export function createPaidCalibrationRequester(config, baseFetch = globalThis.fetch) {
    ...
    return (body, idempotencyKey) => {
        if (!body || !idempotencyKey)
            throw new Error("Calibration body and idempotency key are required");
