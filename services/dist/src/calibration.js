...
export function calibrate(evidence, request) {
    ...
    for (let index = 0; index < points.length; index += 1) {
        ...
        if (compare(point.close, peak) > 0)
            peak = point.close;
        maxDrawdownBps = Math.max(maxDrawdownBps, downsideBps(peak, point.low));
        if (index > 0)
            downsideReturns.push(downsideBps(points[index - 1].close, point.close));
    }
...
function validateRequest(request) {
    if (request.chainId <= 0n)
        throw new Error("Invalid policy chain ID");
    ...
    if (!Number.isSafeInteger(request.issuedAt) || request.issuedAt <= 0)
        throw new Error("Invalid policy issuance time");
    if (!Number.isSafeInteger(request.validUntil)
...
function parseDecimal(value) {
    ...
    if (!match)
        throw new Error("Invalid decimal evidence");
    const fraction = (match[2] ?? "").replace(/0+$/, "");
    if (fraction.length > 36)
        throw new Error("Decimal evidence exceeds supported precision");
    const coefficient = BigInt(`${match[1]}${fraction}`);
    if (coefficient <= 0n)
        throw new Error("Price evidence must be positive");
...
function downsideBps(previous, current) {
    const [before, after] = align(previous, current);
    if (after >= before)
        return 0;
    return Number(ceilDiv((before - after) * 10000n, before));
}
