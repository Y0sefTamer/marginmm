...
import { z } from "zod";
import { calibrate, canonicalJson, MAX_POLICY_TTL, } from "./calibration.js";
import { AAVE_V3_ETHEREUM_SUBGRAPH_ID, fetchGraphEvidence, UNISWAP_V3_ETHEREUM_SUBGRAPH_ID, } from "./graph.js";
import { signCalibrationArtifact } from "./policy-artifact.js";
...
export class CalibrationIdempotencyStore {
    ...
    #entries = new Map();
    #journalPath;
    constructor(maxEntries = 512, journalPath) {
        this.maxEntries = maxEntries;
        if (!Number.isSafeInteger(maxEntries) || maxEntries <= 0)
            throw new Error("Invalid idempotency capacity");
        if (journalPath !== undefined && !journalPath.trim())
            throw new Error("Idempotency journal path is invalid");
        this.#journalPath = journalPath;
    }
    static load(journalPath, maxEntries = 512) {
        ...
        if (entries.length > maxEntries)
            throw new Error("Calibration idempotency journal exceeds capacity");
        for (const [id, entry] of entries) {
            const value = entry;
            if (!requestId.safeParse(id).success || !value || typeof value !== "object" || Array.isArray(value)
                ...
                throw new Error("Calibration idempotency journal is invalid");
            store.#entries.set(id, value);
        }
        return store;
    }
    inspect(requestId, requestHash) {
        ...
        if (entry.state === "pending")
            return "pending";
        if (!entry.response)
            throw new Error("Invalid completed idempotency entry");
    ...
    reserve(requestId, requestHash) {
        ...
        if (this.#entries.size >= this.maxEntries)
            throw new Error("Calibration idempotency capacity is exhausted");
    ...
    complete(requestId, requestHash, response) {
        const entry = this.#entries.get(requestId);
    ...
    release(requestId, requestHash) {
        const entry = this.#entries.get(requestId);
        if (entry?.state !== "pending" || entry.requestHash !== requestHash)
            return;
        this.#entries.delete(requestId);
        try {
            this.#persist();
...
function readJournalCandidate(path) {
    try {
...
export function parseCalibrationApiRequest(value) {
    return calibrationRequestSchema.parse(value);
}
export function calibrationRequestHash(request) {
    return keccak256(toUtf8Bytes(canonicalJson(request)));
}
export async function issueCalibration(config, request, dependencies = {}) {
    const queriedAt = (dependencies.wallClockSeconds ?? (() => Math.floor(Date.now() / 1_000)))();
    if (!Number.isSafeInteger(queriedAt) || queriedAt <= 0)
        throw new Error("System clock is invalid");
    const evidence = await fetchGraphEvidence(config.graph, queriedAt, dependencies.fetchImpl ?? globalThis.fetch);
    ...
    const signed = await signCalibrationArtifact(calibrated.artifact, config.calibrationSignerPrivateKey, config.calibrationSignerAddress, issuedAt);
    return {
...
export async function createCalibrationApp(config, dependencies = {}) {
    ...
    app.post("/calibrate", (request, response, next) => {
        try {
            ...
            if (reservation !== "reserved")
                throw new Error("Calibration idempotency reservation changed unexpectedly");
            ...
            response.once("finish", () => {
                ...
                if (response.statusCode === 402) {
                    try {
                        store.release(parsed.requestId, requestHash);
                    }
                    catch { /* fail closed */ }
                }
            });
    ...
    app.post("/calibrate", async (_request, response, next) => {
        const parsed = response.locals.calibrationRequest;
        const requestHash = response.locals.calibrationRequestHash;
        try {
...
export function loadCalibrationServiceConfig(env = process.env) {
    const chainId = required(env, "MARGINMM_CHAIN_ID");
    if (chainId !== CALIBRATION_CHAIN_ID.toString())
        throw new Error("MARGINMM_CHAIN_ID must be 31337 for the MVP demo");
...
function validateServiceConfig(config) {
    ...
    if (!config.idempotencyPath.trim())
        throw new Error("Calibration idempotency journal path is invalid");
    if (!Number.isSafeInteger(config.port) || config.port < 1_024 || config.port > 65_535) {
        throw new Error("Calibration port is invalid");
    }
}
export async function readLatestChainTimestamp(ethereumRpcUrl) {
    validateLocalRpcUrl(ethereumRpcUrl);
    ...
    try {
        ...
        if (network.chainId !== CALIBRATION_CHAIN_ID)
            throw new Error("Calibration RPC must use chain 31337");
...
function validateLocalRpcUrl(value) {
    let url;
    try {
        url = new URL(value);
    }
    catch {
        throw new Error("Calibration RPC URL is invalid");
    }
