import { getAddress, keccak256, Signature, toUtf8Bytes, TypedDataEncoder, verifyTypedData, Wallet, } from "ethers";
...
export const MARKET_POLICY_TYPES = {
    MarketPolicy: [
...
export async function signCalibrationArtifact(artifact, calibrationPrivateKey, expectedSigner, now) {
    validateArtifactShape(artifact, now);
    const wallet = new Wallet(calibrationPrivateKey);
    if (wallet.address !== getAddress(expectedSigner))
        throw new Error("Calibration private key does not match configured signer");
...
export function validateSignedCalibration(signed, context) {
    validateArtifactShape(signed.artifact, context.now);
    if (signed.artifact.chainId !== context.chainId
    ...
    if (digest !== signed.digest)
        throw new Error("Calibration artifact digest mismatch");
...
export function validateSignedCalibrationEvidence(signed, canonicalEvidence, context) {
...
export function validateArtifactShape(artifact, now) {
    if (artifact.chainId <= 0n)
        throw new Error("Invalid artifact chain ID");
    ...
    if (![1, 2, 3].includes(artifact.marketRegime))
        throw new Error("Unsupported market regime");
    ...
    if (artifact.modelVersion !== MODEL_VERSION)
        throw new Error("Unsupported calibration model version");
    ...
    if (!Number.isSafeInteger(artifact.evidenceBlockFrom)
