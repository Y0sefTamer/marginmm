import { Agent, OpenAIProvider, Runner, tool, type Model, type RunContext } from "@openai/agents";
...
import {
  ...
  CALIBRATION_PAIR,
  CALIBRATION_PROFILE,
  calibrationRequestHash,
  parseCalibrationApiRequest,
  readLatestChainTimestamp,
  type CalibrationApiRequest,
...
import {
  validateSignedCalibrationEvidence,
  type SignedCalibration,
} from "./policy-artifact.js";
import {
  createPaidCalibrationRequester,
  HEDERA_TESTNET,
  type HbarPaymentConfig,
} from "./x402.js";
...
interface RiskAgentState {
  ...
  dependencies: RiskAgentDependencies;
  policyStatus?: PolicyStatus;
  refreshNeeded?: boolean;
  graphEvidence?: GraphEvidenceSummary;
  outgoingRequest?: CalibrationApiRequest;
  paymentAttempts: number;
  paymentMayHaveBeenAttempted: boolean;
  paymentReceipt?: HederaPaymentReceipt;
  calibrationPayload?: unknown;
  validatedCalibration?: CalibrationApiResponse;
...
export function assessRefreshNeed(status: PolicyStatus, forceRefresh: boolean): boolean {
  return forceRefresh || status.state !== "valid";
}

export function validateHederaSettlement(
  receipt: SettleResponse,
  expectedAmount: string,
): HederaPaymentReceipt {
...
export function validateCalibrationApiResponse(
  value: unknown,
  expectedRequest: CalibrationApiRequest,
  config: Pick<RiskAgentConfig, "policyRegistry" | "pairId" | "calibrationSignerAddress">,
  chainTimestamp: number,
  ...
  const signed: SignedCalibration = {
  ...
  return parsed as CalibrationApiResponse;
}

function validateEvidenceMetadata(parsed: z.infer<typeof responseSchema>, signed: SignedCalibration): void {
  ...
  const blockTo = Math.min(
    metadata.sources.market.lastSwapBlock,
    metadata.sources.market.indexedBlock.number,
    metadata.sources.aave.indexedBlock.number,
  );
  if (
    metadata.sources.market.firstSwapBlock !== signed.artifact.evidenceBlockFrom
...
function signedGraphEvidenceSummary(calibration: CalibrationApiResponse): GraphEvidenceSummary {
  let evidence: unknown;
  try { evidence = JSON.parse(calibration.canonicalEvidence); }
  catch { throw new Error("Canonical calibration evidence is not JSON"); }
  const metadata = evidenceMetadataSchema.parse(evidence);
  return {
...
export async function runRiskAgent(
  config: RiskAgentConfig,
  dependencies: RiskAgentDependencies = {},
): Promise<RiskPolicyProposal> {
  let state: RiskAgentState | undefined;
  ...
  try {
    ...
    const agent = new Agent<RiskAgentState>({
    ...
    if (error instanceof RiskAgentWorkflowError) throw error;
...
function deterministicAgentDecision(state: RiskAgentState): z.infer<typeof agentOutputSchema> {
...
function createRiskAgentTools() {
  ...
  return [
    tool({
      ...
      execute: async (_input, runContext?: RunContext<RiskAgentState>) => {
        const state = requiredState(runContext);
        if (state.policyStatus) throw new Error("Policy status was already read");
    ...
    tool({
      ...
      parameters: noArguments,
      execute: (_input, runContext?: RunContext<RiskAgentState>) => {
        const state = requiredState(runContext);
        if (!state.policyStatus) throw new Error("Policy status must be read first");
        if (state.refreshNeeded !== undefined) throw new Error("Refresh need was already assessed");
    ...
    tool({
      ...
      execute: async (_input, runContext?: RunContext<RiskAgentState>) => {
        const state = requiredState(runContext);
        if (state.refreshNeeded !== true) throw new Error("Graph evidence is only loaded for a required refresh");
        if (state.graphEvidence) throw new Error("Graph evidence was already loaded");
    ...
    tool({
      ...
      execute: async (_input, runContext?: RunContext<RiskAgentState>) => {
        ...
        if (state.paymentAttempts !== 0) throw new Error("The payment attempt limit for this run was reached");
        state.paymentAttempts = 1;
        if (state.policyStatus.latestPolicyVersion >= 0xffff_ffff) throw new Error("Policy version space is exhausted");
        ...
        if (!response.ok) throw new Error(`Paid calibration request failed with HTTP ${response.status}`);
        const paymentHeader = response.headers.get("payment-response");
        const replay = response.headers.get("x-marginmm-idempotent-replay") === "true";
        if (paymentHeader) {
          state.paymentReceipt = validateHederaSettlement(
            decodePaymentResponseHeader(paymentHeader),
            state.config.priceTinybar,
          );
        } else if (replay) {
    ...
    tool({
      ...
      execute: async (_input, runContext?: RunContext<RiskAgentState>) => {
        ...
        state.validatedCalibration = validateCalibrationApiResponse(
...
async function readPolicyStatus(config: RiskAgentConfig): Promise<PolicyStatus> {
  const provider = new JsonRpcProvider(config.ethereumRpcUrl);
  const network = await provider.getNetwork();
  if (network.chainId !== 31337n) throw new Error("Risk Agent Ethereum reader is restricted to chain 31337");
  const block = await provider.getBlock("latest");
  if (!block) throw new Error("Local Ethereum chain returned no latest block");
...
async function readJsonWithLimit(response: Response, limit: number): Promise<unknown> {
  const declared = response.headers.get("content-length");
  if (declared && Number(declared) > limit) throw new Error("Calibration response exceeds the size limit");
  if (!response.body) throw new Error("Calibration response has no body");
...
function validateRiskAgentConfig(config: RiskAgentConfig): void {
  ...
  if (!bytes32Pattern.test(config.pairId) || /^0x0{64}$/.test(config.pairId)) throw new Error("Invalid pairId");
  if (!bytes32Pattern.test(config.strategy) || /^0x0{64}$/.test(config.strategy)) throw new Error("Invalid strategy");
  const floor = BigInt(config.makerHardFloorStressHF);
  if (floor < 1_010_000_000_000_000_000n || floor > 3_000_000_000_000_000_000n) {
    throw new Error("Maker hard floor is outside contract bounds");
  }
  if (
    !Number.isSafeInteger(config.refreshLeadSeconds)
    || config.refreshLeadSeconds < 0
    || config.refreshLeadSeconds > MAX_POLICY_TTL
  ) throw new Error("Invalid refresh lead time");
  if (
    !Number.isSafeInteger(config.policyValiditySeconds)
    || config.policyValiditySeconds < 1
    || config.policyValiditySeconds > MAX_POLICY_TTL
  ) throw new Error("Invalid policy validity period");
  if (!config.modelApiKey.trim()) throw new Error("Agent model API key is required");
  ...
  if (!config.model.trim()) throw new Error("Agent model is required");
}

function requiredState(runContext?: RunContext<RiskAgentState>): RiskAgentState {
  if (!runContext) throw new Error("Risk Agent run context is missing");
  return runContext.context;
}

function safeNumber(value: unknown, field: string): number {
  const result = Number(BigInt(value as bigint));
  if (!Number.isSafeInteger(result) || result < 0) throw new Error(`${field} is outside the supported range`);
  return result;
}

function uint32Number(value: unknown, field: string): number {
  const result = safeNumber(value, field);
  if (result > 0xffff_ffff) throw new Error(`${field} is outside uint32`);
  return result;
}
