...
export async function fetchGraphEvidence(
  config: GraphEvidenceConfig,
  nowSeconds: number,
  fetchImpl: Fetch = globalThis.fetch,
): Promise<GraphEvidence> {
  if (!Number.isSafeInteger(nowSeconds) || nowSeconds <= 0) throw new Error("Invalid evidence timestamp");
  ...
  const [marketRaw, aaveRaw] = await Promise.all([
    ...
    queryGraph(
      config.aave,
      AAVE_QUERY,
      { assets: [WETH_ADDRESS, USDC_ADDRESS], from: fromTimestamp },
      timeoutMs,
      fetchImpl,
    ),
  ]);
...
function endpoint(source: GraphSource): string {
  if (!source.apiKey.trim()) throw new Error("Graph API key is required");
...
async function queryGraph(
  ...
  if (!response.ok) throw new Error(`Graph gateway request failed with HTTP ${response.status}`);
  const payload = await readJsonWithLimit(response, MAX_GRAPH_RESPONSE_BYTES);
  const envelope = z.object({
    data: z.unknown().optional(),
    errors: z.array(z.object({ message: z.string() })).optional(),
  }).parse(payload);
  if (envelope.errors?.length) throw new Error(`Graph query failed: ${envelope.errors[0]?.message ?? "unknown error"}`);
  if (envelope.data === undefined) throw new Error("Graph query returned no data");
  return envelope.data;
}

async function readJsonWithLimit(response: Response, limit: number): Promise<unknown> {
  const declared = response.headers.get("content-length");
  if (declared && Number(declared) > limit) throw new Error("Graph response exceeds the size limit");
  if (!response.body) throw new Error("Graph response has no body");
...
function validateMarketEvidence(evidence: MarketEvidence, expectedPool: string): void {
  if (evidence._meta.hasIndexingErrors) throw new Error("Market subgraph reports indexing errors");
  if (!evidence.pool || evidence.pool.id !== expectedPool) throw new Error("Configured market pool was not indexed");
  const tokens = new Set([evidence.pool.token0.id, evidence.pool.token1.id]);
  if (!tokens.has(WETH_ADDRESS) || !tokens.has(USDC_ADDRESS)) throw new Error("Market pool is not WETH/USDC");
  if (evidence.poolHourDatas.length < 25) throw new Error("Insufficient hourly market observations");
  if (evidence.poolHourDatas.length === 1000) throw new Error("Market evidence is truncated");
...
function validateAaveEvidence(evidence: AaveEvidence): void {
  if (evidence._meta.hasIndexingErrors) throw new Error("Aave subgraph reports indexing errors");
  if (evidence.liquidationCalls.length === 1000) throw new Error("Aave liquidation evidence is truncated");
