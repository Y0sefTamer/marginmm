  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Dashboard sections">
        ...
        <a className="nav-link" href="#position"><Icon name="wallet" />Position</a>
        <a className="nav-link" href="#policy"><Icon name="sliders" />Risk policy</a>
        <a className="nav-link" href="#activity"><Icon name="activity" />Execution activity</a>
      </nav>
      <div className="sidebar-bottom"><div className="fork-card"><Icon name="flask" /><strong>A local proving ground</strong><p>Explore how collateral and policy shape executable liquidity.</p><span className="tiny-badge">MAINNET FORK</span></div><div className="sidebar-footer"><span className="status-dot" />MarginMM / MVP <span>v0.1</span></div></div>
    </aside>

    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb">Workspace <span>/</span> <strong>Overview</strong></div><div className="topbar-right"><span className="network-pill"><span className="network-icon">◇</span>Mainnet fork</span><span className={`connection-pill ${connected ? 'online' : ''}`}><span className="status-dot" />{connection === 'connecting' ? 'Connecting' : connected ? 'Local API connected' : 'Disconnected'}</span></div></header>
      <main id="overview">
        <div className="page-heading"><div><div className="eyebrow">COLLATERAL-AWARE MARKET MAKING</div><h1>Your liquidity, within limits.</h1><p>Manage your position. Set your stress floor. Review every fill.</p></div><button className="button subtle refresh-button" onClick={() => void refresh()} disabled={busy !== null}><Icon name="refresh" size={16} />Refresh state</button></div>

        {connection !== 'connected' && <div className={`connection-banner ${connection === 'disconnected' ? 'warning' : ''}`} role="status"><Icon name="link" /><div><strong>{connection === 'connecting' ? 'Connecting to your local environment' : 'Local backend disconnected'}</strong><p>{connection === 'connecting' ? 'Waiting for live position data. Actions will unlock after a valid response.' : connectionError}</p></div>{connection === 'disconnected' && <button className="button small" onClick={() => void refresh()}>Reconnect</button>}</div>}
        ...
        <section className="overview-grid" aria-label="Position health">
          <div className="health-card">
            <div className="card-heading"><span className="eyebrow">POSITION HEALTH</span><span className={`health-tag ${belowFloor ? 'below' : ''}`}>{!connected ? 'Awaiting data' : belowFloor ? 'Below policy floor' : 'Live snapshot'}</span></div>
            <div className="health-values"><div><span className="metric-label">Current HF</span><strong className="health-number">{hf(state?.currentHF)}</strong><span className="health-caption">Aave account health</span></div><div><span className="metric-label">StressHF <span className="stress-indicator">↘</span></span><strong className={`health-number accent ${belowFloor ? 'danger-text' : ''}`}>{hf(state?.stressHF)}</strong><span className="health-caption">Under configured stress</span></div></div>
            <div className="health-meter" aria-label="Stress health factor scale, capped at 2"><div className="meter-track"><span className="meter-progress" style={{ width: `${meterPercent(state?.stressHF)}%` }} />{state && <span className="meter-marker" style={{ left: `${meterPercent(state.riskFloor)}%` }} />}</div><div className="meter-labels"><span>0.00</span><span>Policy floor {fmt(state?.riskFloor, 2)}</span><span>2.00+</span></div></div>
            <div className="health-footnote"><span className="outline-dot" />{state?.stressHF != null ? `${fmt(headroom(state.stressHF, state.riskFloor), 0, 6)} HF headroom relative to policy` : 'Stress metrics are read from the local backend'}</div>
          </div>

          <div className="policy-summary"><div className="card-heading"><span className="eyebrow">ACTIVE POLICY</span><Icon name="sliders" /></div><div className="policy-value">{fmt(state?.riskFloor, 2)}<span>min. StressHF</span></div><p>Outgoing liquidity is bounded by the maker’s collateral and stress policy.</p><a href="#policy">Adjust policy <Icon name="arrow" size={17} /></a><div className="policy-minimum">Demo policy minimum StressHF <strong>1.10</strong></div></div>
        </section>

        <section id="position" className="position-section" aria-labelledby="position-title"><div className="section-heading"><h2 id="position-title">Position balances</h2><span className="meta-text">Aave V3 · {state ? `Block ${fmt(state.blockNumber)}` : 'Waiting for local fork'}</span></div><div className="balance-grid">
          <article className="balance-card"><div className="balance-label"><Token symbol="WETH" /><span>WETH collateral</span><span className="balance-type">SUPPLIED</span></div><div className="balance-amount" title={state?.collateral.weth}>{fmt(state?.collateral.weth, 4, 8)} <small>WETH</small></div><div className="balance-footer">Output capacity · qMax <strong title={state?.qMax.weth}>{fmt(state?.qMax.weth, 4, 8)} WETH</strong></div></article>
          <article className="balance-card"><div className="balance-label"><Token symbol="USDC" /><span>USDC collateral</span><span className="balance-type">SUPPLIED</span></div><div className="balance-amount" title={state?.collateral.usdc}>{fmt(state?.collateral.usdc, 2, 6)} <small>USDC</small></div><div className="balance-footer">Output capacity · qMax <strong title={state?.qMax.usdc}>{fmt(state?.qMax.usdc, 2, 6)} USDC</strong></div></article>
          <article className="balance-card debt-card"><div className="balance-label"><Token symbol="USDC" /><span>USDC debt</span><span className="balance-type">BORROWED</span></div><div className="balance-amount" title={state?.debt.usdc}>{fmt(state?.debt.usdc, 2, 6)} <small>USDC</small></div><div className="balance-footer"><span className="amber-dot" />Included in the stress calculation</div></article>
        </div></section>

        <div className="workspace-grid">
          <section className="panel quote-panel" aria-labelledby="quote-title"><div className="panel-heading"><div><div className="eyebrow">EXECUTION WORKBENCH</div><h2 id="quote-title">Preview a fill</h2></div><span className="tiny-badge">LOCAL CHAIN</span></div>
            <form onSubmit={event => { event.preventDefault(); void getQuote(); }}>
              <label className="field-label" htmlFor="direction">Trade direction <span>Incoming asset → outgoing asset</span></label>
              <div className="direction-control"><select id="direction" value={direction} disabled={disabled} onChange={event => { setDirection(event.target.value as Direction); setAmount(''); setQuote(null); }}><option value="weth-in">WETH in → USDC out</option><option value="usdc-in">USDC in → WETH out</option></select><span className="direction-decor"><Token symbol={direction === 'weth-in' ? 'WETH' : 'USDC'} /><Icon name="arrow" size={16} /><Token symbol={tokenOut} /></span></div>
              <label className="field-label" htmlFor="amount">Requested output <span>Amount leaving the maker</span></label>
              <div className={`amount-input ${amount && validation ? 'invalid' : ''}`}><input id="amount" inputMode="decimal" autoComplete="off" spellCheck={false} value={amount} placeholder={direction === 'weth-in' ? '1000' : '0.1'} disabled={disabled} aria-invalid={!!validation} aria-describedby="amount-help" onChange={event => { setAmount(event.target.value); setQuote(null); }} /><span>{tokenOut}</span></div>
              <div className="amount-presets">{(direction === 'weth-in' ? ['100', '1000', '5000'] : ['0.01', '0.1', '1']).map(value => <button type="button" className="preset" key={value} disabled={disabled} onClick={() => { setAmount(value); setQuote(null); }}>{fmt(value)}</button>)}<button type="button" className="preset max" disabled={disabled || !capacity || compare(capacity, '0') === 0} onClick={() => { if (capacity) { setAmount(capacity); setQuote(null); } }}>Use qMax</button></div>
              <p id="amount-help" className={amount && validation ? 'field-error' : 'field-help'}>{amount && validation ? validation : 'The backend may reduce output to the available capacity.'}</p>
              <div className="capacity-line"><span>Current output capacity <span className="mono">qMax</span></span><strong>{fmt(capacity)} {tokenOut}</strong></div>
              <button type="submit" className="button primary full-width" disabled={disabled || !!validation}>{busy === 'Quoting' ? 'Requesting quote…' : 'Get executable quote'}<Icon name="arrow" size={18} /></button>
            </form>

            <div className={`quote-result ${quote?.status === 'rejected' ? 'rejected' : ''}`} aria-live="polite">
              {quote ? <><div className="result-heading"><strong>{quote.status === 'rejected' ? 'Quote rejected' : expired ? 'Quote expired' : quote.partialFill ? 'Partial fill available' : 'Quote ready'}</strong><span>{expired ? 'Request a new quote' : `${Math.max(0, Math.ceil((Date.parse(quote.expiresAt) - now) / 1000))}s remaining`}</span></div><dl className="quote-details"><div><dt>Requested output</dt><dd>{fmt(quote.requestedOut)} {tokenOut}</dd></div><div><dt>Quoted qMax</dt><dd>{fmt(quote.qMax)} {tokenOut}</dd></div><div><dt>Executable output</dt><dd className="emphasized">{fmt(quote.executableOut)} {tokenOut}</dd></div><div><dt>Quoted StressHF / floor</dt><dd>{hf(quote.stressHF)} / {fmt(quote.riskFloor, 2)}</dd></div></dl>{quote.partialFill && <p className="field-help">Only the executable amount will be submitted for this quote.</p>}<button className="button primary full-width" disabled={disabled || expired || quote.status !== 'ready'} onClick={() => void fillQuote()}>Execute quoted fill <Icon name="arrow" size={18} /></button></> : <div className="quote-empty"><span className="empty-icon"><Icon name="activity" size={21} /></span><strong>{busy === 'Executing' ? 'Waiting for local execution…' : 'Your quote will appear here'}</strong><p>{busy === 'Executing' ? 'Submission is in progress. Do not resend the request.' : 'Review executable output before submitting a fill.'}</p></div>}
            </div>
          </section>

          <div className="right-stack"><section id="policy" className="panel" aria-labelledby="policy-title"><div className="panel-heading"><div><div className="eyebrow">MAKER CONTROLS</div><h2 id="policy-title">Stress policy</h2></div><Icon name="sliders" /></div><p className="panel-description">Choose the minimum stressed health factor your quotes must preserve.</p><form onSubmit={event => { event.preventDefault(); void updatePolicy(); }}><div className="floor-presets">{[['1.10', 'Demo minimum'], ['1.20', 'More headroom'], ['1.30', 'Higher floor']].map(([value, label]) => <button type="button" className={`floor-preset ${isFloorEqual(floor, value) ? 'selected' : ''}`} key={value} disabled={disabled} aria-pressed={isFloorEqual(floor, value)} onClick={() => { dirtyFloor.current = true; setFloor(value); }}><strong>{value}</strong><span>{label}</span></button>)}</div><label className="field-label" htmlFor="floor">Custom floor <span>Minimum 1.10</span></label><div className="policy-input-row"><input id="floor" inputMode="decimal" autoComplete="off" value={floor} disabled={disabled} aria-invalid={!!policyValidation} aria-describedby="floor-help" onChange={event => { dirtyFloor.current = true; setFloor(event.target.value); }} /><button className="button dark" disabled={disabled || !!policyValidation || (state ? isFloorEqual(floor, state.riskFloor) : false)}>{busy === 'Updating policy' ? 'Applying…' : 'Apply policy'}</button></div><p id="floor-help" className={policyValidation ? 'field-error' : 'field-help'}>{policyValidation ?? 'Demo policy minimum StressHF 1.10. Changes invalidate existing quotes.'}</p></form></section>

            <section className="panel scenario-panel" aria-labelledby="scenario-title"><div className="panel-heading"><div><div className="eyebrow">DEMO LAB</div><h2 id="scenario-title">Put capacity to the test</h2></div><span className="flask-icon"><Icon name="flask" /></span></div><p className="panel-description">Withdraw USDC collateral on the local fork, then observe the change in StressHF and qMax.</p><div className="scenario-buttons"><button className="button scenario-withdraw" disabled={disabled} onClick={() => void scenario('withdraw-usdc')}>{busy === 'Withdrawing' ? 'Withdrawing…' : 'Withdraw USDC'}</button><button className="button subtle" disabled={disabled} onClick={() => void scenario('restore')}><Icon name="refresh" size={15} />{busy === 'Restoring' ? 'Restoring…' : 'Restore demo'}</button></div><div className="local-note"><span className="outline-dot" />Scenario actions change the local fork position.</div></section>
          </div>
        </div>

        <section id="activity" className="panel activity-panel" aria-labelledby="activity-title"><div className="panel-heading"><div><h2 id="activity-title">Execution activity</h2><p className="panel-description">Receipts from this browser session</p></div><span className="tiny-badge">{fills.length} RECEIPTS</span></div>{fills.length === 0 ? <div className="activity-empty"><Icon name="activity" size={24} /><div><strong>No executions yet</strong><p>Confirmed fills and rejections will appear here with their requested and executed amounts.</p></div></div> : <div className="table-scroll"><table><thead><tr><th>Status / direction</th><th>Requested</th><th>Executed</th><th>Transaction / local chain</th></tr></thead><tbody>{fills.map(fill => <tr key={fill.quoteId}><td><span className={`receipt-status ${fill.status}`}>{fill.status === 'filled' ? 'Confirmed' : 'Rejected'}</span><small>{fill.direction === 'weth-in' ? 'WETH → USDC' : 'USDC → WETH'}</small>{fill.reason && <small className="field-error">{fill.reason}</small>}</td><td>{fmt(fill.requestedOut)} {outputToken(fill.direction)}</td><td>{fmt(fill.executedOut)} {outputToken(fill.direction)}</td><td><code className="transaction-hash">{fill.transactionHash ?? 'No transaction'}</code><small>Local chain {fill.chainId}</small></td></tr>)}</tbody></table></div>}</section>

        <footer className="page-footer"><div><span className={`status-dot ${connected ? 'live' : ''}`} />{state ? `${state.chain.name} · Chain ${state.chain.id}` : 'Local chain unavailable'}<span className="footer-divider">/</span>Mainnet fork</div><span>{updatedAt && connected ? `Updated ${Math.max(0, Math.floor((now - updatedAt) / 1000))}s ago · Polls every 5s` : 'No live position data'}</span></footer>
