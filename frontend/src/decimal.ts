...
export const inputToken = (direction: Direction) => direction === 'weth-in' ? 'WETH' : 'USDC';
export const outputToken = (direction: Direction) => direction === 'weth-in' ? 'USDC' : 'WETH';
export const inputDecimals = (direction: Direction) => direction === 'weth-in' ? 18 : 6;
...
export function amountError(value: string, direction: Direction): string | null {
  const decimals = inputDecimals(direction);
  if (!isDecimal(value, decimals)) return `Enter a decimal amount with up to ${decimals} decimal places. No exponent notation.`;
  if (parseUnits(value, decimals) === 0n) return 'Maximum input must be greater than zero.';
  return null;
}

export function minimumOutputError(value: string, direction: Direction): string | null {
  const decimals = outputDecimals(direction);
  if (!isDecimal(value, decimals)) return `Enter a minimum output with up to ${decimals} decimal places.`;
  if (parseUnits(value, decimals) === 0n) return 'Minimum output must be greater than zero.';
  return null;
}
...
export function floorError(value: string): string | null {
  ...
  if (compare(value, '1.01') < 0 || compare(value, '3') > 0) return 'Maker StressHF floor must be between 1.01 and 3.00.';
  return null;
}
