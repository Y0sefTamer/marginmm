...
import { amountError, compare, displayDecimal, floorError, headroom, isDecimal, meterPercent, minimumOutputError } from './decimal';

describe('decimal amounts', () => {
  it('preserves wei precision beyond IEEE-754', () => {
    expect(amountError('0.000000000000000001', 'weth-in')).toBeNull();
  ...
  it.each(['-1', '+1', '1e3', 'NaN', 'Infinity', '.1', '01', ' 1', '1.', '1,000', '0x1', '1.0000001'])('rejects invalid USDC amount %s', value => {
    expect(amountError(value, 'usdc-in')).not.toBeNull();
  });
  it('does not round over-precision or overflow', () => {
    expect(amountError('1.0000000000000000001', 'weth-in')).not.toBeNull();
    expect(amountError('0', 'usdc-in')).not.toBeNull();
    expect(isDecimal('9'.repeat(78), 18)).toBe(false);
    expect(amountError('0.000001', 'usdc-in')).toBeNull();
    expect(minimumOutputError('0.000001', 'weth-in')).toBeNull();
    expect(minimumOutputError('0.0000001', 'weth-in')).not.toBeNull();
  });
  it('enforces contract floor bounds while keeping 1.10 as a demo default', () => {
    expect(floorError('1.009999999999999999')).not.toBeNull();
    expect(floorError('1.01')).toBeNull();
    expect(floorError('1.10')).toBeNull();
    expect(floorError('3.000000000000000001')).not.toBeNull();
