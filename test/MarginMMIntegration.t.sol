...
contract MarginMMIntegrationTest is Test {
    ...
    function testFork_PartialFillSettlesActualATokensAndPreservesBothGuards() public {
        ...
        vm.prank(taker);
        (uint256 quoteIn, uint256 quoteOut,) = router.quote(order, aWETH, aUSDC, c.amountOut, data);
        ...
        vm.prank(taker);
        (uint256 filledIn, uint256 filledOut,) = router.swap(order, aWETH, aUSDC, c.amountOut, data);
    ...
    function testFork_ReverseDirectionUsesFullSafeCapacity() public {
        ...
        vm.prank(taker);
        (uint256 amountIn, uint256 amountOut,) = router.swap(order, aUSDC, aWETH, c.amountOut, data);
